		...
		struct node_info *dni, block_t blkaddr, unsigned int *nofs)
{
	...
	if (sum->version != dni->version) {
		...
		f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK, "valid data with mismatched node version, %s:%d\n",
			__func__, __LINE__);
#endif
	}
	...
	for (segno = start_segno; segno < end_segno; segno++) {
		...
		if (type != GET_SUM_TYPE((&sum->footer))) {
			...
			f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK,
				"Inconsistent segment (%u) type [%c, %d] in SSA and SIT, %s:%d\n",
				segno, type, GET_SUM_TYPE((&sum->footer)), __func__, __LINE__);
#endif
			f2fs_stop_checkpoint(sbi, false,
