		...
		int segno, struct f2fs_sit_entry *raw_sit)
{
	...
	if (unlikely(GET_SIT_VBLOCKS(raw_sit) != valid_blocks)) {
		...
		f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK, "Mismatch valid blocks %d vs. %d, %s:%d\n",
			GET_SIT_VBLOCKS(raw_sit), valid_blocks, __func__, __LINE__);
#endif
	...
	if (unlikely(GET_SIT_VBLOCKS(raw_sit) > usable_blks_per_seg
		...
		f2fs_handle_error(sbi, ERROR_INCONSISTENT_SIT);
#ifdef CONFIG_HONOR_F2FS_DSM
		f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK, "Wrong valid blocks %d or segno %u, %s:%d\n",
			GET_SIT_VBLOCKS(raw_sit), segno, __func__, __LINE__);
#endif
		return -EFSCORRUPTED;
	}
	return 0;
