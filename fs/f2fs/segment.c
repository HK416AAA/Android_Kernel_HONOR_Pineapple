...
void f2fs_print_valid_map(struct f2fs_sb_info *sbi, unsigned int segno, struct seq_file *seq) {
	...
	if (seq)
		seq_printf(seq, "[SegMap] segno.%u fragmentation record score=%u validmap print......\n", segno, se->frag_score);
	else
		pr_err("[SegMap] segno.%u fragmentation record score=%u validmap print......", segno, se->frag_score);
	...
	if (seq) {
		seq_printf(seq, "[SegMap] segno.%u fragmentation recalc score=%u validmap finish......\n", segno, f2fs_get_seg_fragscore(sbi, segno));
		seq_printf(seq, "[SegMap]\n");
	} else {
		pr_err("[SegMap] segno.%u fragmentation recalc score=%u validmap finish......", segno, f2fs_get_seg_fragscore(sbi, segno));
		pr_err("[SegMap]");
	}
}
	...
	for (i = 0; i < del_count; i++) {
		...
		if (unlikely(!exist)) {
			...
			del += 1;
 			...
 			se->frag_score += update_seg_fragscore(sbi, blkaddr + i, false);
 			if (unlikely(is_sbi_flag_set(sbi, SBI_CP_DISABLED))) {
 				/*
		...
		if (f2fs_block_unit_discard(sbi) &&
			...
			sbi->discard_blks++;
 #ifdef CONFIG_F2FS_BLKFRAG
 			se->frag_score += update_seg_fragscore(sbi, blkaddr + i, true);
 #endif
        }
