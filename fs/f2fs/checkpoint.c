...
err_out:
	...
	f2fs_set_need_fsck_report(DSM_F2FS_NEED_FSCK, "orphan failed (ino=%x), run fsck to fix, %s:%d\n",
		ino, __func__, __LINE__);
#endif
...
int f2fs_issue_checkpoint(struct f2fs_sb_info *sbi)
{
	...
	if (cprc->f2fs_issue_ckpt) {
#ifdef CONFIG_HN_VIP_THREAD_BUILDIN
		trace_android_rvh_honor_check_task_static_vip(current, &current_vip);
		if (current_vip)
			trace_android_rvh_honor_static_vip_set(cprc->f2fs_issue_ckpt, SET_VIP, &set_vip_ret);
		wait_for_completion(&req.wait);
		if (current_vip)
			trace_android_rvh_honor_static_vip_set(cprc->f2fs_issue_ckpt, CANCEL_VIP, &set_vip_ret);
#else
		wait_for_completion(&req.wait);
#endif
	} else
