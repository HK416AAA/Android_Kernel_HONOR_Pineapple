		...
		struct mm_walk *walk)
{
	...
	if (pte_present(*pte)) {
#ifdef CONFIG_HONOR_VALID_PGTABLE
		if (!pfn_valid(pte_pfn(*pte))) {
			pr_err("%s invalid pte entry, pte=%016llx\r\n",
...
#ifdef CONFIG_HONOR_VALID_PGTABLE
			if (!pfn_valid(swp_offset_pfn(swpent))) {
				pr_err("%s invalid swpent, ent=%016lx\r\n",
					__func__, swpent.val);
				return;
			}
#endif
			   ...
			   struct mm_walk *walk)
{
	...
	if (!pfn_valid(pmd_pfn(*pmd))) {
		pr_err("%s invalid pmd entry, pmd=%016llx\r\n", __func__, pmd_val(*pmd));
		goto out;
	}
#endif
...
static int proc_pid_smaps_simple_show(struct seq_file *m, void *v)
{
	...
	while (vma) {
		...
		if (vma->vm_mm && !is_vm_hugetlb_page(vma)) {
			walk_page_range(vma->vm_mm, vma->vm_start, vma->vm_end, &ops, &mss);
...
static int proc_pid_smaps_simple_open(struct inode *inode, struct file *file)
{
	...
	ret = single_open(file, proc_pid_smaps_simple_show, priv);
	if (ret)
		goto out_free;
...
static int proc_pid_smaps_simple_single_release(struct inode *inode, struct file *file)
{
	struct seq_file *seq = file->private_data;
	struct proc_maps_private *priv = seq->private;
...
const struct file_operations proc_pid_smaps_simple_operations = {
	...
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= proc_pid_smaps_simple_single_release,
};
#endif /* CONFIG_HONOR_SMAPS_SIMPLE */
