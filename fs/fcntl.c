...
static bool rw_hint_valid(u64 hint)
{
	...
	default:
#ifdef CONFIG_HONOR_CACHE_DISK
		if ((int) hint == RWH_WRITE_LIFE_TEMP)
			return true;
#endif
		return false;
	}
...
#ifdef CONFIG_HONOR_CACHE_DISK
static void fcntl_rw_hint_no_ptr(struct file *filp, unsigned int cmd,
				 unsigned long arg, long *ret)
{
	struct inode *inode = file_inode(filp);

	switch (cmd) {
	case F_GET_RW_HINT_NO_PTR:
		*ret = inode->i_write_hint;
		break;
	case F_SET_RW_HINT_NO_PTR:
		if (!rw_hint_valid(arg)) {
			*ret = -EINVAL;
			break;
		}

		inode_lock(inode);
		inode->i_write_hint = arg;
		pr_debug("Fcntl rw_hint, ino:%lu, hint:%d, wb:%lu.\n",
			 inode->i_ino, (int)inode->i_write_hint,
			 (unsigned long)inode->i_wb);
		inode_unlock(inode);
		*ret = 0;
		break;
	default:
		...
		struct file *filp)
{
	...
	case F_GET_RW_HINT_NO_PTR:
	case F_SET_RW_HINT_NO_PTR:
		fcntl_rw_hint_no_ptr(filp, cmd, arg, &err);
		break;
#endif
	default:
		break;
