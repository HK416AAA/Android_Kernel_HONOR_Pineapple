...
static int qseecom_ta_count = 0;
static int qseecom_count = 0;
		       ...
		       unsigned int align, gfp_t gfp_mask)
{
...
#ifdef CONFIG_GCMA
		ret = cma_alloc_range(cma, pfn, count, gfp_mask);
#else
	...
	if (strcmp(cma->name, "qseecom_region") == 0) {
		if (ret == -EBUSY) {
			qseecom_count++;
		} else {
			qseecom_count = 0;
	...
	if (strcmp(cma->name, "qseecom_ta_region") == 0) {
		if (ret == -EBUSY) {
			qseecom_ta_count++;
		} else {
			qseecom_ta_count = 0;
	...
	if (ret && !(gfp_mask & __GFP_NOWARN)) {
		...
		if (ret == -EBUSY) {
			...
			if (strcmp(cma->name, "qseecom_ta_region") == 0 && qseecom_ta_count > 10) {
				cma_debug_show_areas(cma);
				panic("cma qseecom_ta_region allocation failed");
			}
			if (strcmp(cma->name, "qseecom_region") == 0 && qseecom_count > 10) {
				cma_debug_show_areas(cma);
				panic("cma qseecom_region allocation failed");
			}
		}
