...
/*
 * The dirty expired interval for inodes
 * whose i_rw_hint was set to RWH_WRITE_LIFE_TEMP
 */
unsigned int dirty_shortlife_expire_interval = 15 * 60; /* seconds */
#endif
...
#ifdef CONFIG_HONOR_CACHE_DISK
extern int f2fs_get_inode_dirty_pages(struct inode *inode);
static unsigned long get_dirty_shortlife_pages_sum(struct bdi_writeback *wb)
{
	struct inode *inode;
	struct super_block *sb = NULL;
	unsigned long wb_dirty_shortlife = 0;
	static unsigned long shortlife_too_many_count = 0;

	if (!list_empty(get_wb_shortlife_dirty_list(wb))) {
		spin_lock(&wb->list_lock);
		list_for_each_entry(inode,
				    get_wb_shortlife_dirty_list(wb),
				    i_io_list) {
			if (!sb &&
			    !strncmp(inode->i_sb->s_type->name, "f2fs", 4)) {
				sb = inode->i_sb;
			}

			if (sb && inode->i_sb == sb)
				wb_dirty_shortlife += (unsigned long)
					      f2fs_get_inode_dirty_pages(inode);
		}
		spin_unlock(&wb->list_lock);
	}

	/* Not ignore shorlife dirty while shortlife dirty pages beyond 100M */
	if (wb_dirty_shortlife > 40 * 1024) {
		++shortlife_too_many_count;
		pr_info("too many shortlife inode, shortlife pages:%lu, "
			"count:%lu\n", wb_dirty_shortlife,
			shortlife_too_many_count);
		return 0;
	}

	return wb_dirty_shortlife;
}
#endif
...
#ifdef CONFIG_HONOR_CACHE_DISK
	unsigned long wb_dirty_shortlife = get_dirty_shortlife_pages_sum(wb);
#endif
...
#ifdef CONFIG_HONOR_CACHE_DISK
	if (dtc->wb_dirty > wb_dirty_shortlife)
		dtc->wb_dirty -= wb_dirty_shortlife;
	else
		dtc->wb_dirty = 0;
#endif
}
...
#ifdef CONFIG_HONOR_CACHE_DISK
	if (reclaimable > wb_dirty_shortlife)
		reclaimable -= wb_dirty_shortlife;
	else
		reclaimable = 0;
#endif
...
#ifdef CONFIG_HONOR_CACHE_DISK
	{
		.procname 	= "dirty_shortlife_expire_secs",
		.data 		= &dirty_shortlife_expire_interval,
