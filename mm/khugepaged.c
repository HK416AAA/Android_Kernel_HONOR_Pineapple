...
#define KHUGEPAGED_BIND_CPUS "0-6"
...
#ifdef CONFIG_HUGEPAGE_POOL
		set_cpus_bind(KHUGEPAGED_BIND_CPUS, khugepaged_thread);
		err = sched_setattr(khugepaged_thread, &attr);
		if (err) {
			pr_warn("%s: failed to set sched attr for khugepaged_thread\n", __func__);
