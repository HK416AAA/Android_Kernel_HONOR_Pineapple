...
#ifdef CONFIG_MEMCG_PROTECT_LRU
		int error = 0;
		memcgp = get_protect_file_memcg(folio, mapping);
		if (!mem_cgroup_disabled())
			error = __mem_cgroup_charge(folio, NULL, gfp, &memcgp);
#else
...
error:
#ifdef CONFIG_MEMCG_PROTECT_LRU
	protect_add_page_cache_rollback(folio);
#endif
	if (charged)
				...
				pgoff_t index, gfp_t gfp)
{
	...
	if (unlikely(ret))
		...
		if (get_folio_attr(folio, mapping) == PAGE_FILE_VIP)
			folio_set_active(folio);
#endif
		folio_add_lru(folio);
	}
	...
	if (flags & WQ_FLAG_EXCLUSIVE) {
		...
		if (flags & WQ_FLAG_CUSTOM) {
			...
			if (key->bit_nr == PG_locked)
				set_page_lock_info(folio_page(key->folio, 0));
#endif
			flags |= WQ_FLAG_DONE;
		}
	}
...
#ifdef CONFIG_DEBUG_PAGE_LOCK
		if (bit_nr == PG_locked)
			set_page_lock_info(folio_page(folio, 0));
#endif
...
void folio_unlock(struct folio *folio)
{
	...
	VM_BUG_ON_FOLIO(!folio_test_locked(folio), folio);
	reset_page_lock_info(folio_page(folio, 0));
...
#ifdef CONFIG_CGROUP_IOLIMIT
		io_read_bandwidth_control(PAGE_SIZE * folio_batch_count(&fbatch));
#endif
		/*
...
static unsigned int choose_proper_ra_pages(struct file *file, struct vm_fault *vmf)
{
	...
	return max_t(unsigned int, ra_pages, 2);
}
#endif
...
#ifdef CONFIG_HN_CGROUP_WORKINGSET
	workingset_pagecache_on_pagefault(file, index);
#endif
...
#endif
#ifdef CONFIG_HONOR_FILEMAP_TRACE
		filemap_trace_pagecache_miss(file, index, 1, index, false);
#endif
