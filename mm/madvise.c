...
#ifdef CONFIG_PROCESS_MADVISE_CHECK_ADJ
#define CACHED_APP_MIN_ADJ 900
#define APP_MIN_ADJ 0
#endif
...
#ifdef CONFIG_PROCESS_MADVISE_CHECK_LOCK
	int contended;
#endif
#ifdef CONFIG_PROCESS_MADVISE_CHECK_ADJ
	short origin_oom_score;
	short new_oom_score;
#endif
...
#ifdef CONFIG_PROCESS_MADVISE_CHECK_ADJ
	origin_oom_score = task->signal->oom_score_adj;
#endif

	while (iov_iter_count(&iter)) {
#ifdef CONFIG_PROCESS_MADVISE_CHECK_LOCK
		contended = mmap_lock_is_contended(current->mm);
		if ((behavior == MADV_PAGEOUT) && contended) {
			ret = -EBUSY;
			break;
		}
#endif
#ifdef CONFIG_PROCESS_MADVISE_CHECK_ADJ
		if (behavior == MADV_COLD || behavior == MADV_PAGEOUT) {
			new_oom_score = task->signal->oom_score_adj;
			if (origin_oom_score >= CACHED_APP_MIN_ADJ && new_oom_score < CACHED_APP_MIN_ADJ && new_oom_score >= APP_MIN_ADJ) {
				pr_info("break by adj changed to non-cached, pid %5d, new_adj %5hd\n", task->pid, new_oom_score);
				ret = -EINVAL;
				break;
			}
		}
