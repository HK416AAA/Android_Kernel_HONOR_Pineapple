	...
	__releases(vmf->ptl)
{
	...
	if (folio_test_anon(folio)) {
		...
		if (folio_test_non_compress(folio))
			folio_clear_non_compress(folio);
#endif
		/*
...
#ifdef CONFIG_FAULT_AROUND_OPTIMIZATION
unsigned long fault_around_bytes __read_mostly =
...
static inline bool should_fault_around(struct vm_fault *vmf)
{
...
#ifdef CONFIG_HN_CGROUP_WORKINGSET
	if(unlikely(task_wscg_monitor(current)))
		return false;
#endif
	...
	if (current->signal->oom_score_adj != 0)
		return false;
	if (current_uid().val < 10000) /* AID_APP */
		return false;
	if (time_is_after_jiffies(INITIAL_JIFFIES + 120 * HZ))
		return false;
#endif
