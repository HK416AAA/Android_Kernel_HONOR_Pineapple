...
#ifdef CONFIG_HME_HOOKS_BUILDIN
...
enum page_attr get_folio_attr(struct folio *folio, struct address_space *mapping)
{
	int gid;
	enum page_attr pattr = PAGE_OTHERS;
	if (!enable_vip_page_cache)
		return pattr;
	if (folio && mapping && folio_is_file_lru(folio) && mapping->host) {
...
static void mark_vip_folio_accessed(struct folio *folio)
{
	if (folio && !folio_test_unevictable(folio) && !folio_test_active(folio)) {
		if (folio_test_lru(folio))
	...
	if (folio_test_morechance(folio))
		folio_clear_morechance(folio);
...
void folio_mark_accessed(struct folio *folio)
{
	...
	if (pattr == PAGE_FILE_VIP) {
		mark_vip_folio_accessed(folio);
		return;
	}
#endif

#ifdef CONFIG_HME_HOOKS_BUILDIN
	trace_android_vh_folio_mark_accessed(folio);
#endif
