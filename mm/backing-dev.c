...
struct list_head noop_backing_list;
#endif
...
#ifdef CONFIG_HONOR_CACHE_DISK
	INIT_LIST_HEAD(get_wb_shortlife_dirty_list(wb));
#endif
...
#ifdef CONFIG_HONOR_CACHE_DISK
	wb = kmalloc(sizeof(*wb) + 2 * sizeof(u64), gfp);
#else
...
#ifdef CONFIG_HONOR_CACHE_DISK
	bdi = kzalloc_node(sizeof(*bdi) + 2 * sizeof(u64), GFP_KERNEL, node_id);
#else
