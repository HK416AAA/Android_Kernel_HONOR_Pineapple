...
#ifdef CONFIG_HN_DC_MODULE
static struct hn_dc_ops g_dev_dc_ops;
#endif
...
#ifdef CONFIG_HN_DC_MODULE
int hn_register_dual_connection(struct hn_dc_ops *ops)
{
	if (ops == NULL)
		return -EINVAL;
...
EXPORT_SYMBOL(hn_register_dual_connection);

int hn_unregister_dual_connection(void)
{
	g_dev_dc_ops.dc_send_copy = NULL;
	g_dev_dc_ops.dc_receive = NULL;
	return 0;
}
EXPORT_SYMBOL(hn_unregister_dual_connection);
#endif
