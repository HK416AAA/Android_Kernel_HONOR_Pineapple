...
static inline bool is_prot_page(struct folio *folio)
{
	return folio_test_protect(folio);
}
#else
static inline bool is_prot_page(struct folio *folio)
{
...
int protect_memcg_move_account(
	struct folio *folio, bool compound,
...
int __mem_cgroup_charge(struct folio *folio, struct mm_struct *mm,
			gfp_t gfp, struct mem_cgroup **memcgp);
				    ...
				    gfp_t gfp_mask)
{
	...
	if (mem_cgroup_disabled())
		return 0;
	return __mem_cgroup_charge(folio, mm, gfp_mask, memcgp);
}
#else
