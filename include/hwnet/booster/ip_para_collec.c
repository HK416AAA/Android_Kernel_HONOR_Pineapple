...
#define APP_QOE_SYNC_CMD_LEN 12
...
static u32 get_current_srtt(struct tcp_sock *tp)
{
	...
	if (tp->bytes_received < tp->content_length && tp->key_flow &&
	    tp->rx_opt.rcv_tsecr)
		rcv_rtt_us = tp->rcv_rtt_est.rtt_us;

	rtt_ms = tp->srtt_us > rcv_rtt_us ? tp->srtt_us : rcv_rtt_us;

	time = jiffies_to_msecs(tcp_jiffies32 - tp->rtt_update_tstamp);
	if (time < TIME_TEN_SEC)
		rtt_ms = rtt_ms > tp->prior_srtt_us ? rtt_ms :
						      tp->prior_srtt_us;
...
{
	if (ipv4_is_loopback(ip_addr) || ipv4_is_multicast(ip_addr) ||
	    ipv4_is_local_multicast(ip_addr) || ipv4_is_lbcast(ip_addr) ||
	    ipv4_is_private_10(ip_addr) || ipv4_is_private_172(ip_addr) ||
	    ipv4_is_private_192(ip_addr) || ipv4_is_linklocal_169(ip_addr))
		return true;
	else
		return false;
}
...
static bool is_private_v6_addr(struct sock *sk)
{
	...
	if ((sk->sk_family == AF_INET6) && (addr_type & IPV6_ADDR_MAPPED)) {
...
static u32 is_first_pkt_of_period(struct sock *sk)
{
	...
	tp = tcp_sk(sk);
	if (tp == NULL || g_app_ctx == NULL || g_app_ctx->report_tstamp == 0)
		return false;
...
static bool match_video_flow(struct sock *sk, struct sk_buff *skb,
			     u16 hook_type)
{
	u32 payloadlen;
	u8 *payload = NULL;
...
static bool match_key_flow(struct key_flow_info *info, struct sock *sk,
			   struct sk_buff *skb, u16 hook_type)
{
	switch (info->type) {
	case VIDEO_FLOW:
...
static bool is_key_flow(struct sock *sk, struct sk_buff *skb, u16 hook_type,
			u8 protocol)
{
	struct tcp_sock *tp = NULL;
	u32 uid;
...
static bool need_update_rtt_ofo(struct sock *sk)
{
	...
	if (!RB_EMPTY_ROOT(&tp->out_of_order_queue)) {
		...
		if (tp->rcv_nxt_ofo != 0 && tp->ofo_tstamp != 0)
			return true;
	}
...
void dec_sk_num_for_qoe(struct sock *sk, int new_state)
{
	...
	if ((old_state == TCP_ESTABLISHED || old_state == TCP_SYN_SENT) &&
	    new_state != TCP_ESTABLISHED && new_state != TCP_SYN_SENT)
...
static void update_sk_rtt_sum_common(struct sock *sk)
{
	...
	tp = tcp_sk(sk);
	tp->prior_srtt_us = (tp->srtt_us > MAX_RTT_US) ? MAX_RTT_US :
							 tp->srtt_us;
...
static void update_sk_rtt_ofo_common(struct sock *sk)
{
	...
	time = jiffies_to_usecs(tcp_jiffies32 - tp->ofo_tstamp) *
	       MULTIPLE_OF_RTT;
	tp->prior_srtt_us = (time > MAX_RTT_US) ? MAX_RTT_US : time;
	tp->rtt_update_tstamp = tcp_jiffies32;
}
 ...
 * PID   N  N  Y  Y
 */
static u32 match_app(struct sock *sk, struct sk_buff *skb, struct tcp_res *stat,
		     u8 protocol, u16 hook_type)
{
	u32 uid;
	...
	if (stat->uid == KEY_FLOW_UID &&
	    is_key_flow(sk, skb, hook_type, protocol))
		return FORGROUND_MACH;
	uid = get_sock_uid(sk);
	if (uid == stat->uid)
		return FORGROUND_MACH;
...
static void set_dev_name(struct sock *sk, const struct sk_buff *skb)
{
	...
	if ((1 << sk->sk_state) & TCPF_SYN_SENT) {
		dev_max_len = strnlen(skb->dev->name, IFNAMSIZ - 1);
		strncpy(sk->wifipro_dev_name, skb->dev->name, dev_max_len);
...
inline bool is_keep_alive_msg(struct sock *sk, struct tcphdr *th, struct tcp_sock *tp, s32 tcp_len)
{
	return (sk->sk_state == TCP_ESTABLISHED &&
		ntohl(th->seq) == tp->snd_nxt - 1 &&
		(tcp_len == 0 || tcp_len == 1) &&
		th->ack == 1 &&
		th->fin != 1 &&
		th->rst != 1);
				     ...
				     struct tcphdr *th, s32 tcp_len)
{
	...
	if (tp->snd_nxt != 0 && before(ntohl(th->seq), tp->snd_nxt) &&
	    (sk->sk_state == TCP_ESTABLISHED || sk->sk_state == TCP_SYN_SENT) &&
	    !is_keep_alive_msg(sk, th, tp, tcp_len)) {
		stat->out_rts_pkt++;
		     ...
		     u8 direction)
{
	...
	} else if (direction == NF_INET_FORWARD ||
		   direction == NF_INET_LOCAL_OUT) {
		stat->out_all_pkt++;
	}
}
...
static void update_tcp_rtt_sk_num(struct sock *sk, struct tcp_res_info *stat,
...
static void process_stat_info_tcp_layer(struct sock *sk,
					struct app_list *pos_selec,
					unsigned int direction, u32 dev_id)
{
	if (dev_id < 0 || dev_id >= CHANNEL_NUM || pos_selec == NULL)
		return;

	if (direction >= NF_INET_NEW_SK_HOOK && direction <= NF_INET_OFO_HOOK) {
		update_tcp_rtt_sk_num(sk, &pos_selec->stat.info[dev_id],
				      direction);
...
static void stat_proces(struct sk_buff *skb, struct sock *sk,
			const struct nf_hook_state *state, u8 protocol,
			u16 payload_len)
{
	struct app_stat *app = NULL;
	...
	pos_selec =
		get_match_node_from_list(sk, skb, cpu, protocol, state->hook);
	if (pos_selec == NULL)
		goto unlock;

	if (state->hook >= NF_INET_TCP_DUPACK_IN_HOOK) {
		process_stat_info_tcp_layer(sk, pos_selec, state->hook, dev_id);
	} else {
...
void update_dupack_num(struct sock *sk, bool is_dupack, unsigned int hook)
{
	if (is_dupack)
#else
void update_dupack_num(struct sock *sk, int num_dupack, unsigned int hook)
{
	if (num_dupack > 0)
#endif
			  ...
			  const struct nf_hook_state *state)
{
	...
	if (iph->protocol == IPPROTO_TCP && is_local_or_lan_addr(sk->sk_daddr))
		return NF_ACCEPT;

	payload_len = ntohs(iph->tot_len) -
		      iph->ihl * TCP_HDR_IHL_QUAD; // 32bits to byte
			  ...
			  const struct nf_hook_state *state)
{
	...
	if (iph->nexthdr == IPPROTO_TCP && is_private_v6_addr(sk))
		return NF_ACCEPT;
...
static void cum_cpus_stat(struct app_stat *cur, struct tcp_collect_res *tcp,
			  u16 cnt)
{
	struct app_list *pos = NULL;
	int i;
...
{
	if (uid == BACKGROUND_UID || uid == DNS_UID || uid == KEY_FLOW_UID)
		return;

	g_app_ctx->fore_app_uid = uid;
}

static void sync_apps_list(struct app_id *id, u16 cnt, struct app_stat *stat)
{
	u16 i;
	bool need_new = true;
	...
	record_fore_app_uid(cnt > 0 ? id[cnt - 1].uid : INVALID_UID.val);
	rm_list(&stat->head);
...
static void process_sync(struct req_msg *msg)
{
	...
	pr_info("[IP_PARA]%s data: %d %d", __func__, req->req_flag,
		req->rpt_cycle);
	spin_lock_bh(&g_app_ctx->lock);
...
static void sync_key_flow(struct key_flow *new_flow)
{
	u32 i;
	struct key_flow *flow = &g_app_ctx->flow;
	flow->cnt = min_t(u32, new_flow->cnt, MAX_KEY_FLOW_TYPE_NUM);
	pr_info("[IP_PARA] %s,cnt=%u", __func__, flow->cnt);
	for (i = 0; i < flow->cnt; i++)
...
static void process_link_property_update(struct link_property_msg *msg)
{
	...
	if (interface_id > DS_INTERFACE_NAME_ID &&
	    interface_id != INVALID_LINK_PROPERTITY_INTERFACE_ID)
		return;
...
static void cmd_process(struct req_msg_head *msg)
{
	...
	if (msg->type == APP_QOE_SYNC_CMD) {
		if (unlikely(msg->len != APP_QOE_SYNC_CMD_LEN)) {
			pr_info("[IP_PARA]receive APP_QOE_SYNC_CMD, msg->len error\n");
			return;
		}
		process_sync((struct req_msg *)msg);
	} else if (msg->type == UPDATE_APP_INFO_CMD) {
		struct req_msg *req = (struct req_msg *)msg;

		if (unlikely(msg->len != sizeof(struct req_msg) +
		    req->req.tcp_req.cnt * sizeof(struct app_id))) {
			pr_info("[IP_PARA]receive UPDATE_APP_INFO_CMD, msg->len error\n");
			return;
		}
		process_app_update(req);
	} else if (msg->type == UPDAT_INTERFACE_NAME) {
		if (unlikely(msg->len != sizeof(struct link_property_msg))) {
			pr_info("[IP_PARA]receive UPDAT_INTERFACE_NAME, msg->len error\n");
			return;
		}
...
msg_process *ip_para_collec_init(notify_event *fn)
{
	...
	g_app_ctx->fn = fn;
	g_app_ctx->cur =
		kmalloc(sizeof(struct app_stat) * CONFIG_NR_CPUS, GFP_KERNEL);
