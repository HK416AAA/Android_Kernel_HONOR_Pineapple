...
static void nl_notify_event(struct res_msg_head *msg)
{
	...
	unsafe_memcpy(&p->msg, msg, msg->len,
...
static void netlink_handle_rcv(struct sk_buff *__skb)
{
	...
	case NL_MSG_REG:
		...
		break;
	case NL_MSG_JNI_REQ:
		req = (struct req_msg_head *)NLMSG_DATA(nlh);
		if (unlikely(req == NULL)) {
			pr_err("req = NULL\n");
			break;
		}
		if (unlikely(nlh->nlmsg_len != sizeof(struct nlmsghdr) + req->len)) {
			pr_err("nlh->nlmsg_len != sizeof(struct nlmsghdr) + req->len\n");
			break;
		}
		process_cmd(req);
		break;
...
static int netlink_handle_init(void)
{
	...
	g_ctx.nlfd =
		netlink_kernel_create(&init_net, NETLINK_OLLIE, &nb_nl_cfg);
