...
#if IS_ENABLED(CONFIG_HONOR_RTG) && IS_ENABLED(CONFIG_HN_QOS_ENABLE)
DECLARE_RESTRICTED_HOOK(android_rvh_sched_get_group_id,
	TP_PROTO(struct task_struct *p, int *ret),
	TP_ARGS(p, ret), 1);
#endif

#if IS_ENABLED(CONFIG_HN_QOS_ENABLE) && IS_ENABLED(CONFIG_HN_QOS_SCHED_BUILDIN)
DECLARE_RESTRICTED_HOOK(android_vh_get_task_qos_lvl,
	TP_PROTO(struct task_struct *p, int *ret),
	TP_ARGS(p, ret), 1);
#endif
