...
#ifdef CONFIG_HYPERHOLD_GKI
static void hyperhold_memcg_iter(int (*iter)(struct mem_cgroup_ext *, void *),
				 void *data)
{
...
static void hyperhold_memcg_iter(int (*iter)(struct mem_cgroup *, void *),
				 void *data)
{
...
#ifdef CONFIG_HYPERHOLD_GKI
void hyperhold_track(struct zram *zram, u32 index, struct mem_cgroup *memcg)
{
	...
	if (unlikely(!memcg))
		return;
	mcg_ext = memcg_ext_from_memcg(memcg);
...
#ifdef CONFIG_HYPERHOLD_SWAPIN_TIMEREDUCE
void hyperhold_delay(int *delay_count)
{
...
static void hyperhold_flush_done(struct hyperhold_entry *io_entry, int err)
{
	...
	if (likely(!err)) {
		hyperhold_flush_cb(priv->scenario, io_entry->manager_private);
...
static void hyperhold_free_pagepool(struct schedule_para *sched)
{
	...
	while (!list_empty(&sched->priv.page_pool.page_pool_list)) {
		free_page =
			list_first_entry(&sched->priv.page_pool.page_pool_list,
					 struct page, lru);
...
static void hyperhold_fill_entry(struct hyperhold_entry *io_entry,
				 struct hyperhold_buffer *io_buf, void *private)
{
	...
	io_entry->io_bdev = io_buf->zram->bdev;
#ifdef CONFIG_HYPERHOLD_FILE
	hyperhold_fill_entry_addr(io_buf->zram, io_entry);
#endif
}
				   ...
				   unsigned long *mcg_reclaim_sz)
{
	...
	if (*require_size < MIN_RECLAIM_ZRAM_SZ) {
		hh_print(HHLOG_INFO,
			 "require_size:%lu < MIN_RECLAIM_ZRAM_SZ \n",
			 *require_size);
		return -EIO;
	}
	...
	*mcg_reclaim_sz = memcg_reclaim_size(memcg);
	...
	if (*mcg_reclaim_sz < MIN_RECLAIM_ZRAM_SZ) {
		hh_print(HHLOG_INFO,
			 "mcg_reclaim_sz:%lu < MIN_RECLAIM_ZRAM_SZ\n",
			 *mcg_reclaim_sz);
		return -EAGAIN;
	}
...
static void hyperhold_stat_alloc_fail(enum hyperhold_scenario scenario, int err)
{
...
static int hyperhold_reclaim_extent(struct mem_cgroup_ext *memcg,
				    struct schedule_para *sched,
				    unsigned long *require_size,
				    unsigned long *mcg_reclaim_sz, int *io_err)
#else
...
#endif
{
	...
	sched->io_entry =
		hyperhold_malloc(sizeof(struct hyperhold_entry), false, true);
	...
	reclaim_size = hyperhold_extent_create(
		memcg, &sched->io_entry->ext_id, &sched->io_buf,
	...
	ret = hyperhold_write_extent(sched->io_handler, sched->io_entry);
	hyperhold_perf_lat_end(&sched->record, HYPERHOLD_IO_EXTENT);
	if (unlikely(ret)) {
		hh_print(HHLOG_ERR, "hyperhold write failed! %d\n", ret);
		*require_size = 0;
		*io_err = ret;
	...
	return hyperhold_update_reclaim_sz(require_size, mcg_reclaim_sz,
					   reclaim_size);
}

#ifdef CONFIG_HYPERHOLD_GKI
static int hyperhold_permcg_reclaim(struct mem_cgroup_ext *memcg, void *data)
#else
int hyperhold_permcg_reclaim(struct mem_cgroup *memcg, void *data)
#endif
{
	/*
	...
	sched->io_handler =
		hyperhold_init_plug(memcg->zram, HYPERHOLD_RECLAIM_IN, sched);
	...
	while (*require_size) {
		if (hyperhold_reclaim_extent(memcg, sched, require_size,
					     &mcg_reclaim_sz, &io_err))
			break;
...
#ifdef CONFIG_HYPERHOLD_GKI
int hyperhold_permcg_can_shrink(struct mem_cgroup_ext *memcg_ext, void *data)
{
	unsigned long nr_anon, nr_zram, nr_eswap, can_shrink, total;
	int score;
	int ub_mem2zram_ratio, ratio;
	struct mem_cgroup *memcg = memcg_from_memcg_ext(memcg_ext);
	struct hh_scan_control *sc = (struct hh_scan_control *)data;

	if (!memcg_ext || !memcg)
		return 0;

	if (hyperhold_memcg_anon_is_small(memcg) && sc->memcg_small_skipped) {
		can_shrink = 0;
		goto done;
	}

	score = hyperhold_get_memcg_score(memcg_ext);
	if (score < 0 || (score == 0 && sc->score_0_skipped)) {
		can_shrink = 0;
		goto done;
	}

	if (!sc->mem2zram_check) {
		can_shrink = hyperhold_get_memcg_size(memcg, HH_INACTIVE_ANON);
	} else {
		nr_anon = hyperhold_get_memcg_size(memcg, HH_ANON);
		...
		ub_mem2zram_ratio = atomic_read(
			&memcg_ext->memcg_reclaimed.ub_mem2zram_ratio);
		ratio = (nr_zram + nr_eswap) * 100 / (total + 1);
		if (ratio >= ub_mem2zram_ratio)
			can_shrink = 0;
		else
			can_shrink = total * ub_mem2zram_ratio / 100 - nr_zram -
				     nr_eswap;
	}
done:
	atomic64_set(&memcg_ext->memcg_reclaimed.nr_can_shrink, can_shrink);
	sc->total_can_shrink += can_shrink;
	return 0;
}

static unsigned long get_app_fair_shrink_size(struct mem_cgroup *memcg,
					      struct hh_scan_control *sc)
{
	struct mem_cgroup_ext *memcg_ext = memcg_ext_from_memcg(memcg);
	int score = 0;
	...
	if (score >= 1000)
		return PAGES_PER_1MB << 2;
	else if (score >= 900)
		return PAGES_PER_1MB << 1;
	else if (score >= 300)
		return PAGES_PER_1MB;
	else if (score == 0) {
		if (sc->score_0_skipped)
			return 0;
		else
			return PAGES_PER_1MB >> 4;
	}
	return 0;
}

static unsigned long memcg_shrink_size(struct mem_cgroup *memcg,
				       struct hh_scan_control *sc)
{
	const unsigned long max_shrink_per_memcg = PAGES_PER_1MB << 5;
	struct mem_cgroup_ext *memcg_ext = memcg_ext_from_memcg(memcg);
	unsigned long nr_to_shrink = sc->nr_to_shrink;
	...
	nr_to_shrink = atomic64_read(&memcg_ext->memcg_reclaimed.nr_can_shrink);
	nr_to_shrink = min(nr_to_shrink, sc->nr_to_shrink);
	nr_to_shrink = min(nr_to_shrink, max_shrink_per_memcg);

	if (sc->fair_shrink)
		nr_to_shrink =
			min(nr_to_shrink, get_app_fair_shrink_size(memcg, sc));
	else
		nr_to_shrink = min(nr_to_shrink, sc->nr_to_shrink);

	nr_to_shrink = min(nr_to_shrink, SWAP_CLUSTER_MAX);

	return nr_to_shrink;
}

/** reclaim anon from memory to zram
 * the caller need to invoke hyperhold_permcg_can_shrink() before.
**/
int hyperhold_permcg_shrink(struct mem_cgroup_ext *memcg_ext, void *data)
{
	struct hh_scan_control *sc = (struct hh_scan_control *)data;
	struct mem_cgroup *memcg = memcg_from_memcg_ext(memcg_ext);
	const unsigned long failed_shrink_threshold = PAGES_PER_1MB >> 5;
	unsigned long nr_to_shrink_memcg, nr_shrinked = 0;
	int ret = 0;

	if (!memcg || !memcg_ext) {
		hh_print(HHLOG_ERR, "sc->mcg is null in permcg_shrink\n");
		goto done;
	}
	...
	if (sc->buffer_check) {
		...
		if (hhgki_buffer_is_suitable()) {
			hh_print(HHLOG_INFO,
				 "permcg_shrink buffer check successed\n");
			ret = -EPERM;
			goto done;
		}
	}
	...
	if (!css_tryget_online(&memcg->css)) {
		hh_print(HHLOG_ERR, "memcg css tryget failed\n");
		goto done;
	}
	nr_to_shrink_memcg = memcg_shrink_size(memcg, sc);
	if (!nr_to_shrink_memcg) {
		hh_print(HHLOG_INFO, "memcg shrink size is 0\n");
		goto done;
	}

	nr_shrinked = try_to_free_mem_cgroup_pages(
		memcg, nr_to_shrink_memcg, GFP_KERNEL, MEMCG_RECLAIM_MAY_SWAP);
	if (nr_shrinked < failed_shrink_threshold) {
		atomic64_set(&memcg_ext->memcg_reclaimed.nr_can_shrink, 0);
		hh_print(HHLOG_INFO,
			 "reclaim anon from memcg failed, size: %lu\n",
			 nr_shrinked);
		goto done;
	}
	hh_print(
		HHLOG_INFO,
		"shrink to zram, memcg: %d, size: %lu, zram_stored_size: %lu\n",
		memcg_id_from_memcg(memcg), nr_shrinked,
		atomic64_read(&memcg_ext->zram_page_size));

	nr_to_shrink_memcg =
		atomic64_read(&memcg_ext->memcg_reclaimed.nr_can_shrink);
	if (nr_to_shrink_memcg > nr_shrinked)
		atomic64_set(&memcg_ext->memcg_reclaimed.nr_can_shrink,
			     (nr_to_shrink_memcg - nr_shrinked));
	else
		atomic64_set(&memcg_ext->memcg_reclaimed.nr_can_shrink, 0);

	css_put(&memcg->css);

	if (sc->total_can_shrink > nr_shrinked)
		sc->total_can_shrink -= nr_shrinked;
	else
		sc->total_can_shrink = 0;

	if (sc->nr_to_shrink > nr_shrinked)
		sc->nr_to_shrink -= nr_shrinked;
	else {
		ret = -EINVAL;
		sc->nr_to_shrink = 0;
	}
	...
	sc->nr_shrinked += nr_shrinked;
	return ret;
}

static void hyperhold_shrink_do_work(struct work_struct *work)
{
	struct async_req *rq = container_of(work, struct async_req, work);
	int old_nice = task_nice(current);
	struct mem_cgroup *memcg = rq->mcg;
	struct mem_cgroup_ext *memcg_ext = memcg_ext_from_memcg(memcg);

	set_user_nice(current, rq->nice);

	/* iterate all memcgs to reclaim pages
	 * invocation to the function hyperhold_permcg_can_shrink() is called
	 * in the upper function.
	 */
	if (!memcg) {
		hyperhold_memcg_iter(hyperhold_permcg_can_shrink,
				     (void *)rq->sc);
		hyperhold_memcg_iter(hyperhold_permcg_shrink, (void *)rq->sc);
	} else {
		hyperhold_permcg_can_shrink(memcg_ext, rq->sc);
		hyperhold_permcg_shrink(memcg_ext, rq->sc);
	}
			    ...
			    struct hh_scan_control *sc)
{
	...
	if (!hyperhold_enable())
		return;
	if (!sc) {
		hh_print(HHLOG_ERR, "scan_control is null\n");
		return;
	}

	rq = hyperhold_malloc(sizeof(struct async_req), false, true);
	if (!rq) {
		hh_print(HHLOG_ERR, "alloc async req failed!\n");
		return;
	}
	...
	rq->mcg = memcg;
	INIT_WORK(&rq->work, hyperhold_shrink_do_work);
	queue_work(hyperhold_get_shrink_workqueue(), &rq->work);
...
static void hyperhold_reclaim_work(struct work_struct *work)
{
	...
	if (!memcg)
		...
		struct mem_cgroup_ext *memcg_ext = memcg_ext_from_memcg(memcg);
		if (memcg_ext)
			hyperhold_permcg_reclaim(memcg_ext, &rq->size);
		else
			hh_print(HHLOG_ERR,
				 "memcg_ext is NULL the memcg is %d\n",
				 memcg_id_from_memcg(memcg));
#else
		hyperhold_permcg_reclaim(memcg, &rq->size);
#endif
	}
...
static unsigned long hyperhold_reclaim(struct mem_cgroup *memcg,
				       unsigned long size)
{
	/*
	...
	if (!memcg)
		...
		if (!memcg_ext)
			return 0;
		out_size = memcg_reclaim_size(memcg_ext);
#else
		out_size = memcg_reclaim_size(memcg);
#endif
	}
...
{
	return hyperhold_reclaim(NULL, size);
}
EXPORT_SYMBOL_GPL(hyperhold_reclaim_in);

unsigned long hyperhold_reclaim_in_memcg(struct mem_cgroup *memcg,
					 unsigned long size)
{
	return hyperhold_reclaim(memcg, size);
}
EXPORT_SYMBOL_GPL(hyperhold_reclaim_in_memcg);

static unsigned long hyperhold_reclaim_sync(struct mem_cgroup *memcg,
					    unsigned long size)
{
	unsigned long out_size, eswap_size;
	...
	eswap_size = out_size = min(out_size, size);
	hh_print(HHLOG_INFO, "eswap out require size %lu", eswap_size);

	hyperhold_reclaimin_inc();
	if (!memcg)
		hyperhold_memcg_iter(hyperhold_permcg_reclaim, &out_size);
	else {
#ifdef CONFIG_HYPERHOLD_GKI
		struct mem_cgroup_ext *memcg_ext = memcg_ext_from_memcg(memcg);
		if (memcg_ext)
			hyperhold_permcg_reclaim(memcg_ext, &out_size);
		else
			hh_print(HHLOG_ERR,
				 "memcg_ext is NULL the memcg is %d\n",
				 memcg_id_from_memcg(memcg));
#else
		hyperhold_permcg_reclaim(memcg, &out_size);
#endif
	}
	hyperhold_reclaimin_dec();

	hh_print(HHLOG_INFO, "after eswapout, the size left is: %lu\n",
		 out_size);

	return out_size == 0 ? eswap_size : (eswap_size - out_size);
}

unsigned long hyperhold_reclaim_in_sync(unsigned long size)
{
	return hyperhold_reclaim_sync(NULL, size);
}
EXPORT_SYMBOL_GPL(hyperhold_reclaim_in_sync);
...
#endif
{
	...
	sched->io_entry->ext_id = hyperhold_find_extent_by_memcg(
		mcg, &sched->io_buf, &sched->io_entry->manager_private);
	hyperhold_perf_lat_end(&sched->record, HYPERHOLD_FIND_EXTENT);
...
#endif
{
	...
	sched = hyperhold_malloc(sizeof(struct schedule_para), !preload,
				 preload);
	if (unlikely(!sched)) {
		hh_print(HHLOG_ERR, "alloc sched failed!\n");
		hyperhold_stat_alloc_fail(HYPERHOLD_BATCH_OUT, -ENOMEM);
	...
	hyperhold_perf_lat_start(&sched->record, HYPERHOLD_INIT);
	sched->io_handler = hyperhold_init_plug(
		mcg->zram, preload ? HYPERHOLD_PRE_OUT : HYPERHOLD_BATCH_OUT,
		sched);
	hyperhold_perf_lat_end(&sched->record, HYPERHOLD_INIT);
	if (unlikely(!sched->io_handler)) {
		hh_print(HHLOG_ERR, "plug start failed!\n");
...
#else
static int hyperhold_do_batch_out(struct mem_cgroup *mcg, unsigned long size,
				  bool preload)
#endif
{
	/*
	...
	ret = hyperhold_plug_finish(sched->io_handler);
	if (unlikely(ret)) {
		hh_print(HHLOG_ERR, "hyperhold read flush failed! %d\n", ret);
		hyperhold_stat_alloc_fail(HYPERHOLD_BATCH_OUT, ret);
	} else {
		ret = io_err;
	}
...
int hyperhold_batch_out(struct mem_cgroup *mcg, unsigned long size,
			bool preload)
{
	int ret;
#ifdef CONFIG_HYPERHOLD_GKI
...
#ifdef CONFIG_HYPERHOLD_GKI
	ret = hyperhold_do_batch_out(memcg_ext, size, preload);
#else
...
static bool hyperhold_fault_out_check(struct zram *zram, u32 index,
	...
	*zentry = zram_get_handle(zram, index);
	...
	while (zram_test_flag(zram, index, ZRAM_BATCHING_OUT)) {
		...
		if (addr != *zentry) {
			hh_print(
				HHLOG_ERR,
				"addr changed, index = %d, old = %lx, new = %lx\n",
				index, *zentry, addr);
			return false;
		}
	}
	...
	if (!hyperhold_cache_out(zram, index, 0, false)) {
		...
		if (likely(atomic_dec_and_test(
			    &zram->table[index].hh_faultout_refcount)))
...
static int hyperhold_fault_out_get_extent(struct zram *zram,
					  struct schedule_para *sched,
					  unsigned long zentry, u32 index)
{
	sched->io_buf.zram = zram;
	sched->priv.zram = zram;
	sched->io_buf.pool = NULL;
	hyperhold_perf_lat_start(&sched->record, HYPERHOLD_FIND_EXTENT);
	sched->io_entry->ext_id = hyperhold_find_extent_by_idx(
		zentry, &sched->io_buf, &sched->io_entry->manager_private);
	hyperhold_perf_lat_end(&sched->record, HYPERHOLD_FIND_EXTENT);
...
#ifdef CONFIG_HYPERHOLD_CACHE
static int hyperhold_fault_out_exit_check(struct zram *zram, u32 index,
					  unsigned long zentry, int ret)
#else
static int hyperhold_fault_out_exit_check(struct zram *zram, u32 index, int ret)
#endif
{
	if (likely(!ret)) {
#ifdef CONFIG_HYPERHOLD_CACHE
		...
		if (unlikely(zram_test_flag(zram, index, ZRAM_WB))) {
			hh_print(HHLOG_ERR, "still in WB status! index = %u\n",
				 index);
			ret = -EIO;
		}
	}
				      ...
				      unsigned long zentry)
{
	...
	hyperhold_perf_lat_start(&sched->record, HYPERHOLD_IOENTRY_ALLOC);
	sched->io_entry =
		hyperhold_malloc(sizeof(struct hyperhold_entry), true, true);
	hyperhold_perf_lat_end(&sched->record, HYPERHOLD_IOENTRY_ALLOC);
	if (unlikely(!sched->io_entry)) {
		hh_print(HHLOG_ERR, "alloc io entry failed!\n");
	...
	hyperhold_perf_lat_end(&sched->record, HYPERHOLD_IO_EXTENT);
	if (unlikely(ret)) {
		hh_print(HHLOG_ERR, "hyperhold read failed! %d, index = %u\n",
			 ret, index);
...
int hyperhold_fault_out(struct zram *zram, u32 index)
{
	...
	zram_slot_unlock(zram, index);
	//TODO check if nofail is correct or not
	sched = hyperhold_malloc(sizeof(struct schedule_para), true, true);

	if (unlikely(!sched)) {
		hh_print(HHLOG_ERR,
			 "alloc sched failed in hyperhold_fault_out!\n");
		hyperhold_stat_alloc_fail(HYPERHOLD_FAULT_OUT, -ENOMEM);
		zram_slot_lock(zram, index);
		return ret;
	}
	...
	hyperhold_perf_lat_start(&sched->record, HYPERHOLD_INIT);
	sched->io_handler =
		hyperhold_init_plug(zram, HYPERHOLD_FAULT_OUT, sched);
	hyperhold_perf_lat_end(&sched->record, HYPERHOLD_INIT);
	if (unlikely(!sched->io_handler)) {
		hh_print(HHLOG_ERR, "plug start failed!\n");
		hyperhold_stat_alloc_fail(HYPERHOLD_FAULT_OUT, -ENOMEM);
		ret = -EIO;
		hyperhold_fail_record(HYPERHOLD_FAULT_OUT_INIT_FAIL, index, 0,
				      sched->record.task_comm);
...
void hyperhold_idle_reclaim(void)
{
	...
	for (memcg = get_next_memcg_ext(NULL); memcg;
	     memcg = get_next_memcg_ext(memcg))
				   ...
				   unsigned long nr_to_shrink)
{
	...
	int loop = 12;
	struct mem_cgroup *mcg;
	struct mem_cgroup_ext *mcg_ext = memcg_ext_from_memcg_id(mcgid);
	...
	do {
		nr_to_shrink_mcg = nr_to_shrink_mcg > SWAP_CLUSTER_MAX ?
					   nr_to_shrink_mcg :
					   SWAP_CLUSTER_MAX;

		nr_shrinked_once = try_to_free_mem_cgroup_pages(
			mcg, nr_to_shrink_mcg, GFP_KERNEL,
