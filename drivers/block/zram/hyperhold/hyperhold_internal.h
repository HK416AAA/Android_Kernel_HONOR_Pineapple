...
enum { HHLOG_ERR = 0, HHLOG_WARN, HHLOG_INFO, HHLOG_DEBUG, HHLOG_MAX };
...
#define hh_print(l, f, ...) (l <= cur_lvl() ? pt(f, ##__VA_ARGS__) : pr_none())
...
#define HYPERHOLD_NAME_LEN 100

#ifdef CONFIG_HYPERHOLD_FILE
#define HP_SPACE_INFO_INITED 0x01
#define HP_SPACE_INFO_FILE_INITED 0x02
#define hp_align_mb(s) ((s) >> 20)

enum hyperhold_space_type {
	HP_NOT_INIT = 0,
	HP_PARTITION_ARCHIVAL = 1,
	HP_PARTITION_FILE_ARCHIVAL,
};
enum hyperhold_sis_status {
	HP_NOT_IN_EXPAND = 0,
	HP_IN_EXPAND,
};

struct hyperhold_file_bdev {
	atomic_t in_expand;
	int fd;
	unsigned long ino;
	struct file *filp;
...
struct hyperhold_stat {
	...
	atomic64_t swapin_time;
	atomic64_t swapout_time;
	atomic64_t bioin_time;
	atomic64_t bioout_time;
...
#ifdef CONFIG_HYPERHOLD_GKI
void hyperhold_manager_memcg_init(struct mem_cgroup *sysmemcg,
				  struct zram *zram);
...
hyperhold_read_mcg_ext_stats(struct mem_cgroup_ext *mcg_ext,
			     enum hyperhold_mcg_member mcg_member);
extern bool hhgki_buffer_is_suitable(void);
...
int hyperhold_cache_out(struct zram *zram, u32 index, unsigned long addr,
			bool unpin);
...
void hyperhold_perf_start(struct hyperhold_key_point_record *record,
			  ktime_t stsrt, unsigned long long start_ravg_sum,
			  enum hyperhold_scenario scenario);

void hyperhold_perf_end(struct hyperhold_key_point_record *record);

void hyperhold_perf_lat_start(struct hyperhold_key_point_record *record,
			      enum hyperhold_key_point type);

void hyperhold_perf_lat_end(struct hyperhold_key_point_record *record,
			    enum hyperhold_key_point type);

void hyperhold_perf_lat_point(struct hyperhold_key_point_record *record,
			      enum hyperhold_key_point type);

void hyperhold_perf_async_perf(struct hyperhold_key_point_record *record,
			       enum hyperhold_key_point type, ktime_t start,
			       unsigned long long start_ravg_sum);

void hyperhold_perf_io_stat(struct hyperhold_key_point_record *record,
			    int page_cnt, int segment_cnt);
...
void hyperhold_fail_record(enum hyperhold_fail_point point, u32 index,
			   int ext_id, unsigned char *task_comm);
...
#define HYPERHOLD_KEY_SIZE 128
#else
#define HYPERHOLD_KEY_SIZE 64
#endif
#define HYPERHOLD_IO_KEY_SIZE 64
extern u8 hyperhold_io_key[HYPERHOLD_KEY_SIZE];
extern u32 hyperhold_io_key_len;
