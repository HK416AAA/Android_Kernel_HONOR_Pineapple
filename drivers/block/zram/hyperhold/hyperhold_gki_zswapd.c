...
atomic64_t hhgki_max_skip_interval = ATOMIC_LONG_INIT(MAX_SKIP_INTERVAL);
...
void hhgki_zswapd_pressure_report(enum hhgki_zswapd_pressure_level level)
{
	...
	if (ret < 0)
		pr_err("HHGKI: SWAP-MM: %s : level:%u, ret:%d ", __func__,
		       level, ret);
}
...
bool hhgki_buffer_is_suitable(void)
{
	u32 curr_buffers = hhgki_calc_sys_cur_avail_buffers();
...
static void hhgki_snapshot_anon_refaults(void)
{
	...
	while ((memcg_ext = get_next_memcg_ext(memcg_ext)))
		memcg_ext->memcg_reclaimed.reclaimed_pagefault =
			hyperhold_read_mcg_ext_stats(memcg_ext,
						     MCG_ANON_FAULT_CNT);
...
static bool hhgki_get_area_anon_refault_status(void)
{
	...
	ratio = (cur_anon_pagefault - hhgki_area_last_anon_pagefault) *
		percent_constant /
		(jiffies_to_msecs(cur_time - hhgki_last_anon_snapshot_time) +
		 1);
...
int hhgki_snapshotd_run(void)
{
	...
	hhgki_snapshotd_task =
		kthread_run(hhgki_snapshotd, NULL, "hhgki_snapshotd");
...
void hhgki_snapshotd_stop(void)
{
	struct task_struct *snapshotd_task =
		hhgki_snapshotd_task; /*lint !e578*/
...
#endif
	diff_buffers =
		hhgki_get_avail_buffers_value() -
		hhgki_calc_sys_cur_avail_buffers(); /* B_target - B_current */
	diff_buffers *= SZ_1M / PAGE_SIZE; /* MB to page */
	diff_buffers *=
		hhgki_get_compress_ratio_value(); /* after_comp to before_comp */
	diff_buffers =
		diff_buffers * percent_constant / nr_total; /* page to ratio */
	/*lint -e666 */
	return min(hhgki_get_zram_wm_ratio_value(),
		   hhgki_get_zram_wm_ratio_value() - diff_buffers);
	/*lint +e666 */
}
...
void hhgki_wakeup_zswapd(void)
{
	...
	if (IS_ERR(hhgki_zswapd_task) || !hhgki_zswapd_task)
		return;
	...
	curr_interval = jiffies_to_msecs(jiffies - hhgki_last_zswapd_time);
...
void hhgki_wake_all_zswapd(void)
{
	...
	mod_timer(&hhgki_zswapd_pause_timer,
		  jiffies + (HZ * zswapd_pause_interval_tmp));
...
static unsigned long zswapd_shrink_memcg(unsigned long nr_to_shrink)
{
	...
	while ((memcg_ext = get_next_memcg_ext(memcg_ext))) {
		...
		for (i = 0; i < NR_INACTIVE_FILE; i++) {
			memcg_anon_size += hyperhold_memcg_page_state_local(
				memcg, NR_LRU_BASE + i);
		}
		...
		hh_print(HHLOG_INFO, "memcg: %d, anon_size is:%lu\n",
			 memcg_id_from_memcg_ext(memcg_ext), memcg_anon_size);
		...
		nr_to_shrink_mcg =
			total * ub_mem2zram_ratio / 100 - (nr_zram + nr_eswap);
		nr_shrinked += hyperhold_mcg_shrink(
			memcg_id_from_memcg_ext(memcg_ext), nr_to_shrink_mcg);

		hh_print(HHLOG_INFO,
			 "memcg:%s id:%d, ext_id:%d, nr_shrinked: %lu\n",
			 memcg_ext->name, memcg_id_from_memcg_ext(memcg_ext),
			 memcg_ext_id_from_memcg_ext(memcg_ext), nr_shrinked);
...
static void zswapd_shrink(void)
{
	...
	if (nr_reclaimed < hhgki_get_empty_round_check_threshold_value()) {
		...
		if (hhgki_last_round_is_empty)
			hhgki_zswapd_skip_interval = min(
				hhgki_zswapd_skip_interval * increase_rate,
	...
	hh_print(
		HHLOG_INFO,
		"HHGKI: nr_reclaimed: %lu ,hhgki_zswapd_skip_interval: %llu ,hhgki_last_round_is_empty: %d\n",
		nr_reclaimed, hhgki_zswapd_skip_interval,
		hhgki_last_round_is_empty);
}
...
static int hhgki_zswapd(void *p)
{
	...
	while (!kthread_should_stop()) {
		...
		if (hhgki_get_area_anon_refault_status()) {
			...
			hh_print(HHLOG_INFO,
				 "hhgki_zswapd is ZSWAPD_REFAULT\n");
			goto do_eswap;
		}
		...
		if (hhgki_anon_size_is_low()) {
			low_anon = true;
			hh_print(HHLOG_INFO,
				 "hhgki_zswapd hhgki_anon_size_is_low\n");
			goto do_eswap;
		}
				      ...
				      const char *buf, size_t len)
{
	...
	if (sscanf(buf, "%u,%u,%u,%llu", &avail_buffers_value,
		   &min_avail_buffers_value, &high_avail_buffers_value,
...
ssize_t
hyperhold_area_anon_refault_threshold_store(struct device *dev,
					    struct device_attribute *attr,
					    const char *buf, size_t len)
{
	u64 val;
...
ssize_t
hyperhold_empty_round_check_threshold_store(struct device *dev,
					    struct device_attribute *attr,
					    const char *buf, size_t len)
{
	u64 val;
...
ssize_t hyperhold_anon_refault_snapshot_min_interval_store(
						  ...
						  const char *buf, size_t len)
{
	...
	buf = skip_spaces(buf);
	if (sscanf(buf, "%hu,%u,%u,%u", &memcg_id, &ub_mem2zram_ratio,
		   &ub_zram2ufs_ratio, &refault_threshold) != params_num)
		return -EINVAL;

	memcg_ext = memcg_ext_from_memcg_id(memcg_id);

	if (ub_mem2zram_ratio > HYGKI_MAX_RATIO ||
	    ub_zram2ufs_ratio > HYGKI_MAX_RATIO || !memcg_ext)
		return -EINVAL;
				       ...
				       const char *buf, size_t len)
{
	...
	if (unlikely(ret)) {
		hh_print(HHLOG_ERR,
			 "hyperhold_compress_ratio_store is error!\n");
		return EINVAL;
	}
					    ...
					    char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE,
			 "buffer size: %u MB, recent refault: %llu\n", buffers,
			 hhgki_global_anon_refault_ratio);
}
...
{
	return scnprintf(buf, PAGE_SIZE,
			 "pause_zswapd_interval: %u\n"
			 "pause_zswapd_stat: %u\n",
			 atomic_read(&hhgki_pause_zswapd_interval),
			 atomic_read(&hhgki_pause_zswapd));
}

ssize_t hyperhold_area_anon_refault_threshold_show(
...
ssize_t hyperhold_empty_round_check_threshold_show(
	struct device *dev, struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%lld\n",
...
ssize_t hyperhold_anon_refault_snapshot_min_interval_show(
	struct device *dev, struct device_attribute *attr, char *buf)
{
	return scnprintf(
		buf, PAGE_SIZE, "%lld\n",
						 ...
						 char *buf)
{
	...
	while ((memcg_ext = get_next_memcg_ext(memcg_ext))) {
		size += scnprintf(buf + size, PAGE_SIZE, "%d,%d,%d,%d,%d\n",
				  memcg_id_from_memcg_ext(memcg_ext),
				  memcg_ext_id_from_memcg_ext(memcg_ext),
				  memcg_ext->memcg_reclaimed.ub_mem2zram_ratio,
				  memcg_ext->memcg_reclaimed.ub_zram2ufs_ratio,
				  memcg_ext->memcg_reclaimed.refault_threshold);
	}
