					 ...
					 bool fast, bool nofail)
{
	...
	struct io_extent *io_ext =
		hyperhold_malloc(sizeof(struct io_extent), fast, nofail);
	...
	for (i = 0; i < (int)EXTENT_PG_CNT; i++) {
		io_ext->pages[i] =
			hyperhold_alloc_page(pool, GFP_ATOMIC, fast, nofail);
		if (!io_ext->pages[i]) {
			  ...
			  unsigned long eswpentry, int size)
{
	...
	if (size < 0 || size > (int)PAGE_SIZE) {
		hh_print(HHLOG_ERR, "eswpentry = %lx ,size = %d invalid\n",
			 eswpentry, size);
		return;
	}
...
#ifdef CONFIG_HYPERHOLD_GKI
static bool zram_test_skip(struct zram *zram, u32 index,
			   struct mem_cgroup_ext *mcg)
#else
...
#endif
{
	...
	if (zram_test_flag(zram, index, ZRAM_SAME))
		return true;
#ifdef CONFIG_CLOSE_SHMEM_HYPERHOLD_OUT
	if (zram_test_flag(zram, index, ZRAM_NO_SWAPOUT))
		return true;
#endif
...
#endif
{
	...
	if (index >= (u32)zram->area->nr_objs) {
		hh_print(HHLOG_ERR, "index = %u invalid\n", index);
		return;
	}
	if (!mcg) {
	...
	if (mcg) {
		hh_print(
			HHLOG_DEBUG,
			"moved to UFS: memcg_id: %d,memcg_ext_id: %d,size: %d,after_moved hyperhold_stored_size: %d",
			memcg_id_from_memcg_ext(mcg),
			memcg_ext_id_from_memcg_ext(mcg), size,
			   ...
			   struct io_extent *io_ext)
{
	...
	if (mcg) {
		hh_print(
			HHLOG_DEBUG,
			"moved_to_zram: memcg_id: %d,memcg_ext_id: %d,size: %d, after_moved hyperhold_stored_size: %d\n",
			memcg_id_from_memcg_ext(mcg),
			memcg_ext_id_from_memcg_ext(mcg), size,
			atomic64_read(&mcg->hyperhold_stored_size));
	} else {
...
static int move_to_zram(struct zram *zram, u32 index, struct io_extent *io_ext)
{
	...
	if (!handle) {
		hh_print(
			HHLOG_ERR,
			"alloc handle failed, index = %u, eswpentry = %lx, size = %d\n",
			index, eswpentry, size);
		return -ENOMEM;
	}
...
static void extent_unlock(struct io_extent *io_ext)
{
	...
	struct mem_cgroup *sys_memcg = NULL;
#else
...
#ifdef CONFIG_HYPERHOLD_GKI
	sys_memcg = memcg_from_memcg_ext(mcg);
	if (mcg && sys_memcg)
		css_put(&sys_memcg->css);
#else
	if (mcg)
		css_put(&mcg->css);
#endif
		       ...
		       enum hyperhold_scenario scenario)
{
	...
	io_ext->cnt =
		zram_rmap_get_extent_index(zram->area, ext_id, io_ext->index);
...
static int shrink_entry_list(struct io_extent *io_ext)
{
	...
	zram = mcg->zram;
#ifdef CONFIG_HYPERHOLD_GKI
	hh_print(HHLOG_DEBUG, "memcg_id: %d,memcg_ext_id: %d\n",
		 memcg_id_from_memcg_ext(mcg),
...
int hyperhold_cache_out(struct zram *zram, u32 index, unsigned long eswpentry,
			bool unpin)
{
	u8 *dst = NULL;
	unsigned long handle;
...
int hyperhold_move_cache(struct zram *zram, int nr_exts)
{
	...
	hh_print(HHLOG_DEBUG, "run move cache, total = %lld, nr = %d\n",
		 atomic64_read(&zram->area->cache_move_runs), nr_exts);

	radix_tree_for_each_slot(slot, &zram->area->cache->tree, &iter, 0) {
		if (extent_can_move(zram->area, iter.index)) {
			ext_id = get_extent(zram->area, iter.index);
			if (ext_id < 0)
				continue;
			if (move_cache_to_zram(zram, iter.index)) {
				moved++;
				if (moved >= nr_exts)
					break;
			}
	...
	hh_print(HHLOG_DEBUG, "moved extent %d, total = %lld\n", moved,
...
int hyperhold_set_cache_level(struct zram *zram, int level)
{
	...
	zram->area->reclaim_in_cache_enable = level &
					      HYPERHOLD_CACHE_RECLAIM_IN;

	return 0;
}

int hyperhold_set_cache_wm(struct zram *zram, int wm, bool is_high)
{
	if (!zram || !zram->area)
		return -EINVAL;

	if (is_high)
		zram->area->cache_high_wm = wm;
	else
		zram->area->cache_low_wm = wm;
...
void hp_cache_show(struct seq_file *m, struct zram *zram)
{
	...
	seq_printf(m, "hyperhold_cache_pages: %lu\n",
	...
	seq_printf(m, "hyperhold_cache_shrink_time: %lld\n",
		   atomic64_read(&zram->area->cache_shrink_time));
	...
	seq_printf(m, "hyperhold_cache_move_exts: %lld\n",
		   atomic64_read(&zram->area->cache_move_exts));
	seq_printf(m, "hyperhold_cache_move_time: %lld\n",
		   atomic64_read(&zram->area->cache_move_time));
}
...
void hyperhold_cache_state(struct zram *zram, char *buf, ssize_t *size)
{
	...
	*size += scnprintf(
		...
		"Hyperhold cache shrink runs: %lld, reclaim page: %lld, shrink time: %lld\n",
		atomic64_read(&area->cache_shrinker_runs),
		atomic64_read(&area->cache_shrinker_reclaim_pages),
		atomic64_read(&area->cache_shrink_time));
	*size += scnprintf(
		buf + *size, PAGE_SIZE - *size,
		"Hyperhold cache move runs: %lld, move exts: %lld, move time: %lld\n",
		atomic64_read(&area->cache_move_runs),
		atomic64_read(&area->cache_move_exts),
		atomic64_read(&area->cache_move_time));
	*size += scnprintf(buf + *size, PAGE_SIZE - *size,
			   "Hyperhold cache watermark: %d %d\n",
			   area->cache_high_wm, area->cache_low_wm);
}
#endif
/*
...
void hyperhold_manager_deinit(struct zram *zram)
{
	...
	zram->area = NULL;
	hh_print(HHLOG_ERR, "free hyperhold area\n");
}
...
int hyperhold_manager_init(struct zram *zram)
{
	...
	hh_print(HHLOG_ERR, "alloc hyperhold area succ\n");
	return 0;
out:
...
{
	unsigned int memcg_ext_id;
	struct hyperhold_area *area = zram->area;
	struct mem_cgroup_ext *mcg = memcg_ext_from_memcg(sysmcg);
	...
	memcg_ext_id = alloc_bitmap(area->mcg_ext_bitmap, area->nr_mcgs, 1);
	if (memcg_ext_id <= 0) {
		hh_print(HHLOG_ERR, "alloc memcg_ext id failed\n");
		spin_unlock(&area->mcg_ext_lock);
		return;
	}
	area->mcg_ext_rmap[memcg_ext_id] = (unsigned long)(mcg);
	spin_unlock(&area->mcg_ext_lock);
	mcg->mcg_ext_id = memcg_ext_id;
	hh_print(HHLOG_DEBUG, "memcg id:%d ,memcg_ext id:%d\n",
		 memcg_id_from_memcg(sysmcg), memcg_ext_id_from_memcg(sysmcg));

	hh_list_init(mcg_idx(zram->area, memcg_ext_id), zram->area->obj_table);
	hh_list_init(mcg_idx(zram->area, memcg_ext_id), zram->area->ext_table);
#else
...
#ifdef CONFIG_HYPERHOLD_GKI
	hh_print(HHLOG_ERR,
		 "new memcg_ext in zram, memcg_id: %d, memcg_ext_id: %d.\n",
		 memcg_id_from_memcg(sysmcg), memcg_ext_id_from_memcg(sysmcg));
#else
	hh_print(HHLOG_ERR, "new memcg in zram, name:%s, id = %d.\n", mcg->name,
		 mcg->id.id);
#endif
}
...
void hyperhold_manager_memcg_deinit(struct mem_cgroup *mcg)
{
	...
	hh_print(HHLOG_DEBUG, "deinit memcg_id: %d, memcg_ext_id: %d\n",
		 memcg_id_from_memcg_ext(mcg),
		 memcg_ext_id_from_memcg_ext(mcg));
#else
	mcg_id = mcg->id.id;
	hh_print(HHLOG_ERR, "deinit mcg_id = %d\n", mcg->id.id);
#endif
	zram = mcg->zram;
	area = zram->area;
	while (1) {
		...
		if (last_index == index) {
			...
			if (atomic64_read(&mcg->zram_page_size) == 0) {
				hh_print(HHLOG_ERR,
					 "mcg %d do not have page in zram\n",
					 mcg_id);
				hh_print(HHLOG_ERR, "mcg id %d mcg name %s\n",
					 mcg_id, mcg->name);
				break;
			}
	...
	hh_print(HHLOG_ERR, "deinit mcg %d, extent done\n", mcg_id);
}
/*
...
#endif
{
	...
	if (!mcg) {
		hh_print(HHLOG_ERR, "NULL mcg, index = %u\n", index);
		return;
	}
				      ...
				      void **private)
{
	...
	mcg_id = memcg_ext_id_from_memcg_ext(mcg);
	sys_memcg = memcg_from_memcg_ext(mcg);
	if (sys_memcg && !css_tryget_online(&sys_memcg->css)) {
#else
	mcg_id = mcg->id.id;
...
#ifdef CONFIG_HYPERHOLD_GKI
	hh_print(HHLOG_DEBUG, "memcg_id = %d, memcg_ext_id = %d, ext_id = %d\n",
		 memcg_id_from_memcg_ext(mcg), mcg_id, io_ext->ext_id);
#else
	...
	zram = mcg->zram;
	area = zram->area;
	ext_cnt = area->cache->nr_cached_exts;
	if (ext_cnt >= hp_cache_ext_wm(area->nr_exts, area->cache_high_wm)) {
		ktime_t move_cache_time = ktime_get();
		int nr_to_move = ext_cnt - hp_cache_ext_wm(area->nr_exts,
							   area->cache_low_wm);
		int moved = hyperhold_move_cache(zram, nr_to_move);

		atomic64_add(
			ktime_to_us(ktime_sub(ktime_get(), move_cache_time)),
			&mcg->zram->area->cache_move_time);
		hh_print(HHLOG_DEBUG, "to move = %d, moved = %d", nr_to_move,
			 moved);
	}
#endif
...
void hyperhold_extent_register(void *private)
{
	...
	ret = hyperhold_cache_insert(io_ext->zram->area->cache, io_ext->ext_id,
				     ext, false, true);
	if (ret) {
...
#ifdef CONFIG_HYPERHOLD_CACHE
	atomic64_sub(size, &zram->area->ext_stored_size[ext_id]);
	if (!atomic_dec_and_test(&zram->area->ext_stored_pages[ext_id]))
		return;
	ext_id = get_extent(zram->area, ext_id);
#else
				 ...
				 struct hyperhold_buffer *buf, void **private)
{
	...
	if (!io_ext) {
		hh_print(HHLOG_ERR,
			 "io_ext alloc failed, eswpentry = %lu, extid = %d\n",
			 eswpentry, ext_id);
		put_extent(zram->area, ext_id);
		return -ENOMEM;
	}
	...
	io_ext->zram = zram;
#ifdef CONFIG_HYPERHOLD_GKI
	memcg_ext = memcg_ext_from_memcg_ext_id(hh_list_get_mcgid(
		ext_idx(zram->area, ext_id), zram->area->ext_table));
	io_ext->mcg = memcg_ext;
	sys_memcg = memcg_from_memcg_ext(memcg_ext);
	if (memcg_ext && sys_memcg && !css_tryget_online(&sys_memcg->css))
#else
	io_ext->mcg = get_mem_cgroup(hh_list_get_mcgid(
		ext_idx(zram->area, ext_id), zram->area->ext_table));
...
#ifdef CONFIG_HYPERHOLD_GKI
	hh_print(HHLOG_DEBUG, "memcg_id = %d, memcg_ext_id = %d, ext = %d\n",
		 memcg_id_from_memcg(sys_memcg), mcg_id, ext_id);
#else
...
void hyperhold_extent_destroy(void *private, enum hyperhold_scenario scenario)
{
	...
	atomic64_add(
		atomic_read(
			&io_ext->zram->area->ext_stored_pages[io_ext->ext_id]),
 ...
 */
void hyperhold_extent_exception(enum hyperhold_scenario scenario, void *private)
{
	struct io_extent *io_ext = private;
#ifdef CONFIG_HYPERHOLD_GKI
	struct mem_cgroup_ext *mcg = NULL;
	...
	struct mem_cgroup *mcg = NULL;
#endif
	unsigned int op = (scenario == HYPERHOLD_RECLAIM_IN) ? REQ_OP_WRITE :
							       REQ_OP_READ;
...
void memcg_idle_count(struct zram *zram)
{
	...
	for (memcg_ext = get_next_memcg_ext(NULL); memcg_ext;
	     memcg_ext = get_next_memcg_ext(memcg_ext))
		hh_print(HHLOG_ERR,
			 "[%d:%s] idle = %ld, zfault = %ld, hfault = %ld\n",
			 memcg_ext_id_from_memcg_ext(memcg_ext),
			 memcg_ext->name,
			 atomic64_read(&memcg_ext->zram_idle_page),
			 atomic64_read(&memcg_ext->hyperhold_allfaultcnt),
			 atomic64_read(&memcg_ext->hyperhold_faultcnt));
...
void memcg_idle_count(struct zram *zram)
{
	...
	for (mcg = get_next_memcg(NULL); mcg; mcg = get_next_memcg(mcg))
		hh_print(HHLOG_ERR,
			 "[%d:%s] idle = %lld, zfault = %lld, hfault = %lld\n",
			 mcg->id.id, mcg->name,
