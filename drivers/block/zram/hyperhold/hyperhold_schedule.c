...
u8 hyperhold_io_key[HYPERHOLD_KEY_SIZE];
u32 hyperhold_io_key_len;
...
static void hyperhold_key_init(void)
{
	...
	int err;
#endif
#ifndef CONFIG_HYPERHOLD_SWAPIN_TIMEREDUCE
	...
	err = blk_crypto_init_key(&blk_key, hyperhold_io_key,
				  hyperhold_io_key_len,
				  BLK_CRYPTO_KEY_TYPE_HW_WRAPPED,
				  BLK_ENCRYPTION_MODE_AES_256_XTS,
				  DIV_ROUND_UP(lblk_bits, 8), UFS_SECTOR_SIZE);
	if (err) {
		hh_print(HHLOG_ERR, "Error initializing blk-crypto key %d",
			 err);
		return;
	}
...
static void hyperhold_bio_encryption_proc(struct bio *bio)
{
	...
	bio->mas_bio.index = bio->bi_iter.bi_sector >>
			     HYPERHOLD_KEY_INDEX_SHIFT;
#ifdef CONFIG_SCSI_UFS_ENHANCED_INLINE_CRYPTO_V3
				   ...
				   struct hyperhold_segment *segment)
{
	...
	for (i = 0; i < segment->extent_cnt; ++i) {
		...
		req->io_para.done_callback(segment->io_entries_fifo[i], -EIO);
	}
	kfree(segment);
}
...
static void hyperhold_wait_io_finish(struct hyperhold_io_req *req)
{
	...
	if (req->io_para.scenario == HYPERHOLD_FAULT_OUT) {
		...
		wait_for_completion_timeout(&req->io_end_flag,
...
static void hyperhold_inflight_dec(struct hyperhold_io_req *req, int num)
{
	if ((atomic_sub_return(num, &req->extent_inflight) <
	     HYPERHOLD_MAX_INFILGHT_NUM) &&
	    req->limit_inflight_flag && wq_has_sleeper(&req->io_wait))
		wake_up(&req->io_wait);
}
...
static void hyperhold_io_entry_proc(struct hyperhold_segment *segment)
{
	...
	for (i = 0; i < segment->extent_cnt; ++i) {
		...
		hh_print(HHLOG_DEBUG, "extent_id[%d] %d page_num %d\n", i,
			 segment->io_entries_fifo[i]->ext_id, page_num);
...
{
	hh_print(HHLOG_ERR, "io err, ext_id = %d\n", ext_id);
	if (req->io_para.scenario == HYPERHOLD_FAULT_OUT)
				  ...
				  struct hyperhold_segment *segment)
{
	...
	hyperhold_file_io_fail(req->io_para.scenario,
...
static void hyperhold_io_end_work(struct work_struct *work)
{
	...
	ktime_t work_start, swap_time;
	bool is_reclaim_in = false;
	...
	if (is_reclaim_in)
		atomic64_add(ktime_to_us(ktime_sub(ktime_get(), swap_time)),
			     &stat->swapout_time);
	else
		atomic64_add(ktime_to_us(ktime_sub(ktime_get(), swap_time)),
			     &stat->swapin_time);
...
static void hyperhold_end_io(struct bio *bio)
{
	...
	workqueue = (req->io_para.scenario == HYPERHOLD_RECLAIM_IN) ?
			    hyperhold_proc_write_workqueue :
			    hyperhold_proc_read_workqueue;
	segment->time.end_io = ktime_get();
	segment->bio_result = bio->bi_status;

	if (req->io_para.scenario == HYPERHOLD_RECLAIM_IN)
		atomic64_add(ktime_to_us(ktime_sub(ktime_get(),
						   segment->time.submit_bio)),
			     &stat->bioout_time);
	else
		atomic64_add(ktime_to_us(ktime_sub(ktime_get(),
						   segment->time.submit_bio)),
...
static bool hyperhold_ext_merge_back(struct hyperhold_segment *segment,
				     struct hyperhold_entry *io_entry)
{
	struct hyperhold_entry *tail_io_entry = list_last_entry(
		&segment->io_entries, struct hyperhold_entry, list);
...
static bool hyperhold_ext_merge_front(struct hyperhold_segment *segment,
				      struct hyperhold_entry *io_entry)
{
	struct hyperhold_entry *head_io_entry = list_first_entry(
		&segment->io_entries, struct hyperhold_entry, list);
				...
				struct hyperhold_entry *io_entry)
{
	...
	if (segment->io_bdev != io_entry->io_bdev)
		return false;
...
static struct bio *hyperhold_bio_alloc(enum hyperhold_scenario scenario)
{
	...
	struct bio *bio = bio_alloc(NULL, BIO_MAX_PAGES, 0, gfp);

	if (!bio && (scenario == HYPERHOLD_FAULT_OUT))
		bio = bio_alloc(NULL, BIO_MAX_PAGES, 0, GFP_NOIO);
				  ...
				  struct hyperhold_segment *segment)
{
	...
	list_for_each_entry_safe(io_entry, tmp, &segment->io_entries, list) {
		for (i = 0; i < io_entry->pages_sz; i++) {
			...
			if (unlikely(!bio_add_page(bio, io_entry->dest_pages[i],
						   PAGE_SIZE, 0))) {
				return -EIO;
			}
...
static int hyperhold_submit_bio(struct hyperhold_segment *segment)
{
	...
	struct bio *bio = NULL;
	struct block_device *bdev = segment->io_bdev;
				      ...
				      struct hyperhold_entry *io_entry)
{
	...
	segment->io_bdev = io_entry->io_bdev;
	segment->segment_sector = io_entry->addr;
	req->segment = segment;
...
static int hyperhold_io_submit(struct hyperhold_io_req *req, bool merge_flag)
{
	int ret;
	struct hyperhold_segment *segment = req->segment;
...
static bool hyperhold_check_entry_err(struct hyperhold_entry *io_entry)
{
	int i;
	...
	if (unlikely((!io_entry->dest_pages) || (io_entry->ext_id < 0) ||
			       ...
			       struct hyperhold_entry *io_entry)
{
	...
	if (unlikely(hyperhold_check_entry_err(io_entry))) {
		hyperhold_io_err_record(HYPERHOLD_FAULT_OUT_IO_ENTRY_PARA_FAIL,
					req,
					io_entry ? io_entry->ext_id : -EINVAL);
	...
	hh_print(HHLOG_DEBUG, "ext id %d, pages_sz %d, addr %llx\n",
		 io_entry->ext_id, io_entry->pages_sz, io_entry->addr);
	...
	if (unlikely(ret)) {
		hh_print(HHLOG_ERR, "submit fail %d, ext id = %d\n", ret,
			 io_entry->ext_id);
		req->io_para.done_callback(io_entry, -EIO);
...
int hyperhold_schedule_init(void)
{
	...
	hyperhold_proc_read_workqueue = alloc_workqueue(
		"proc_hyperhold_read", WQ_HIGHPRI | WQ_UNBOUND, 0);
	if (unlikely(!hyperhold_proc_read_workqueue))
		return -EFAULT;

	hyperhold_proc_write_workqueue =
		alloc_workqueue("proc_hyperhold_write", WQ_CPU_INTENSIVE, 0);
...
#ifdef CONFIG_HYPERHOLD_FILE
	if (hyperhold_expand_workqueue_init())
		return -EFAULT;
#endif
...
void *hyperhold_plug_start(struct hyperhold_io *io_para)
{
	...
	gfp = (io_para->scenario != HYPERHOLD_RECLAIM_IN) ? GFP_ATOMIC :
							    GFP_NOIO;
...
int hyperhold_read_extent(void *io_handler, struct hyperhold_entry *io_entry)
{
	return hyperhold_io_extent(io_handler, io_entry);
}

/* io_handler validity guaranteed by the caller */
int hyperhold_write_extent(void *io_handler, struct hyperhold_entry *io_entry)
{
	return hyperhold_io_extent(io_handler, io_entry);
}
