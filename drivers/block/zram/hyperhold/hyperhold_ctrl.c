...
unsigned long zram_zsmalloc(struct zs_pool *zs_pool, size_t size, gfp_t gfp)
{
	...
	unsigned long ret;
	struct zs_ext_para ext_para;
	int ext_flag = is_ext_pool(zs_pool);

	if (unlikely(!ext_flag))
		return zs_malloc_ext(zs_pool, size, gfp, ext_flag);
...
#ifdef CONFIG_HYPERHOLD_FILE
ssize_t hyperhold_space_reset_store(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t len)
{
	hyperhold_set_reclaim_in_enable(false);

	hyperhold_exit_fbdev();

	hh_print(HHLOG_ERR, "hyperhold_file_deinit\n");
	return len;
}
			      ...
			      const char *buf, size_t len)
{
	...
	ret = hyperhold_split_param(buf, len);
			     ...
			     char *buf)
{
	...
	*buf = '\0';
	...
	snprintf(buf, PAGE_SIZE,
		 "hyperhold_space:%s\n"
		 "hyperhold_space_type:%d\n"
		 "hyperhold_total_size:%llu MB\n"
		 "hyperhold_partition_size:%llu MB\n",
		 hyperhold_is_file_arch() ? "partition_file" : "partition",
		 atomic_read(&global_settings.space_type),
		 hp_align_mb(hyperhold_get_total_sz()),
		 hp_align_mb(hyperhold_get_partition_sz()));

	hyperhold_file_show(buf, len);
...
#ifdef CONFIG_HYPERHOLD_GKI
static void hyperhold_meminfo_show(void *data, struct seq_file *m)
{
#ifdef CONFIG_HYPERHOLD_FILE
	if (hyperhold_user_closed())
		return;
#endif
}
#endif
...
void hyperhold_init(struct zram *zram)
{
	...
	first_init = (!zram->bdev) && hyperhold_global_setting_init(zram);
	up_write(&zram->init_lock);
	if (!first_init)
		return;

	hh_print(HHLOG_INFO, "bdev %d first %d", (bool)zram->bdev, first_init);
	if (mapper_name != NULL) {
	...
	bind_info.nr_pages = hyperhold_get_total_sz() >> PAGE_SHIFT;
finish_file:
#endif
	...
	memcg_ext_init();
	hhgki_zswapd_init();
	register_trace_android_vh_meminfo_proc_show(hyperhold_meminfo_show,
						    NULL);
#endif
...
static inline struct zram *hyperhold_dev_to_zram(struct device *dev)
{
			       ...
			       size_t len)
{
	int ret;
	unsigned long val;
	struct zram *zram = hyperhold_dev_to_zram(dev);
...
void hyperhold_gki_zram_exit(void)
{
	...
	global_settings.zram = NULL;
	up_write(&zram->init_lock);
	unregister_trace_android_vh_meminfo_proc_show(hyperhold_meminfo_show,
						      NULL);
#ifdef CONFIG_ZSWAPD_GKI
	hhgki_zswapd_deinit();
#endif
