...
/**
 * copy interface info from binder_buffer to target ptr
 */
int honor_binder_alloc_do_copy_from_buffer(struct binder_alloc *alloc,
				       struct binder_buffer *buffer,
				       binder_size_t buffer_offset,
				       void *ptr,
				       size_t bytes)
{
	if (alloc == NULL || buffer == NULL || ptr == NULL) {
		return -EINVAL;
	}
