				...
				enum xhci_ep_reset_type reset_type)
{
	...
	if (ep->ep_state & EP_HALTED) {
		xhci_err(xhci, "Reset ep command for ep_index %d already pending\n",
			 ep->ep_index);
		return 0;
	}
		...
		struct xhci_transfer_event *event)
{
	...
	if (!ep_ring) {
		...
		case COMP_INVALID_STREAM_ID_ERROR:
			xhci_err(xhci, "Stream transaction error ep %u no id\n",
				 ep_index);
	...
	case COMP_STOPPED:
		xhci_err(xhci, "Stopped on Transfer TRB for slot %u ep %u\n",
			 slot_id, ep_index);
		break;
	case COMP_STOPPED_LENGTH_INVALID:
		xhci_err(xhci,
	...
	case COMP_STALL_ERROR:
		xhci_err(xhci, "Stalled endpoint for slot %u ep %u\n", slot_id,
			 ep_index);
		status = -EPIPE;
		break;
	case COMP_SPLIT_TRANSACTION_ERROR:
		xhci_err(xhci, "Split transaction error for slot %u ep %u\n",
			 slot_id, ep_index);
		status = -EPROTO;
		break;
	case COMP_USB_TRANSACTION_ERROR:
		xhci_err(xhci, "Transfer error for slot %u ep %u on endpoint\n",
			 slot_id, ep_index);
		status = -EPROTO;
		break;
	case COMP_BABBLE_DETECTED_ERROR:
		xhci_err(xhci, "Babble error for slot %u ep %u on endpoint\n",
			 slot_id, ep_index);
		status = -EOVERFLOW;
		break;
	...
	case COMP_RING_UNDERRUN:
		...
		if (!list_empty(&ep_ring->td_list))
			xhci_err(xhci, "Underrun Event for slot %d ep %d "
	...
	case COMP_RING_OVERRUN:
		...
		if (!list_empty(&ep_ring->td_list))
			xhci_err(xhci, "Overrun Event for slot %d ep %d "
					"still with TDs queued?\n",
				 TRB_TO_SLOT_ID(le32_to_cpu(event->flags)),
				 ep_index);
		goto cleanup;
	...
	do {
		...
		if (list_empty(&ep_ring->td_list)) {
			...
			if (!(trb_comp_code == COMP_STOPPED ||
			      ...
			      ep_ring->last_td_was_short)) {
				xhci_err(xhci, "WARN Event TRB for slot %d ep %d with no TDs queued?\n",
						TRB_TO_SLOT_ID(le32_to_cpu(event->flags)),
						ep_index);
			}
			if (ep->skip) {
				ep->skip = false;
				xhci_err(xhci, "td_list is empty while skip flag set. Clear skip flag for slot %u ep %u.\n",
					 slot_id, ep_index);
			}
		...
		if (ep->skip && td_num == 0) {
			ep->skip = false;
			xhci_err(xhci, "All tds on the ep_ring skipped. Clear skip flag for slot %u ep %u.\n",
				 slot_id, ep_index);
			goto cleanup;
		}
