		...
		unsigned int length, int result, unsigned int partial)
{
	...
	case -EPIPE:
		...
		if (usb_pipecontrol(pipe)) {
			...
			dev_err(&us->pusb_dev->dev, "-- stall on control pipe\n");
#endif
		...
		usb_stor_dbg(us, "clearing endpoint halt for pipe 0x%x\n",
			     pipe);
#ifdef CONFIG_USB_HONOR_LOG_DEBUG
		dev_err(&us->pusb_dev->dev, "clearing endpoint halt for pipe 0x%x\n",
			     pipe);
#endif
...
#ifdef CONFIG_USB_HONOR_LOG_DEBUG
		dev_err(&us->pusb_dev->dev, "-- babble\n");
#endif
...
#ifdef CONFIG_USB_HONOR_LOG_DEBUG
		dev_err(&us->pusb_dev->dev, "-- transfer cancelled\n");
#endif
		return USB_STOR_XFER_ERROR;
...
#ifdef CONFIG_USB_HONOR_LOG_DEBUG
		dev_err(&us->pusb_dev->dev, "-- abort or disconnect in progress\n");
#endif
		return USB_STOR_XFER_ERROR;
...
#ifdef CONFIG_USB_HONOR_LOG_DEBUG
		dev_err(&us->pusb_dev->dev, "-- unknown error\n");
#endif
		return USB_STOR_XFER_ERROR;
	}
}
	...
	if (result < 0)
		...
		if (test_bit(US_FLIDX_DISCONNECTING, &us->dflags)) {
			...
			usb_stor_dbg(us, "usb_reset_device returns %d\n",
				     result);
#ifdef CONFIG_USB_HONOR_LOG_DEBUG
			dev_err(&us->pusb_dev->dev, "usb_reset_device returns %d\n",
				     result);
#endif
		}
