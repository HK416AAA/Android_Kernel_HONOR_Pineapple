...
#define GOODIX_FRAME_NORMALIZATION_ADDR_DEFAULT_GT9916 0x10BBC
#define GOODIX_FRAME_NORMALIZATION_ADDR_DEFAULT_GT9966 0x1036C
...
#define GOODIX_DEBUG_INFO_ADDR 0x1007F
...
/* GT7986 default addr */
#define GOODIX_GESTURE_ADDR_DEFAULT_GT7986 0x10274
#define GOODIX_SPI_SDO_SEL_ADDR 0xC981
#define GOODIX_SPI_SDO_SEL_REG 0x70
#define GOODIX_DELAY_5MS_7986 5
...
#define BT_CONNECT_STATUS_CHECK_ADDR_GT7986 0x10219
...
#define FULLAOD_SLIDE_POSITION_REPORT 0x80000000
...
#define REG_GET_ACTIVE_DOZE_TIME_ADDR_GT9916 0x10210
#define REG_GET_ACTIVE_DOZE_TIME_ADDR_GT9966 0x10364
...
struct goodix_device_info {
	...
	unsigned int force_power_off_state;
	/*
...
static int touch_driver_gesture_event_9897(struct thp_device *tdev,
					   unsigned int *gesture_wakeup_value);
static int
touch_driver_gesture_event_for_gt9897_dual(struct thp_device *tdev,
					   unsigned int *gesture_wakeup_value);
...
static int touch_driver_spi_read_for_9897(struct thp_device *tdev, u8 *tx_buf,
					  u8 *rx_buf,
					  struct spi_transfer *xfers,
					  struct spi_message *spi_msg,
					  unsigned int addr, unsigned int len);
static int touch_driver_spi_write_for_9897(struct thp_device *tdev, u8 *tx_buf,
					   unsigned int addr,
					   struct spi_transfer *xfers, u8 *data,
					   unsigned int len);
...
/* gt9966 spi read */
static int touch_driver_spi_read_for_9966(struct thp_device *tdev, u8 *tx_buf,
					  u8 *rx_buf,
					  struct spi_transfer *xfers,
					  struct spi_message *spi_msg,
					  unsigned int addr, unsigned int len)
				 ...
				 u8 *data, unsigned int len)
{
	...
	if (tdev->thp_core->support_vendor_ic_type == GT9897) {
		if ((len + GOODIX_READ_WRITE_BYTE_OFFSET_GT9897) >
		   ...
		   tdev->thp_core->support_vendor_ic_type == GT7986) {
	...
	if (cd->support_vendor_ic_type == GT9896) {
		...
		touch_driver_spi_read_for_9897(tdev, tx_buf, rx_buf, &xfers[0],
					       &spi_msg, addr, len);
	} else if (cd->support_vendor_ic_type == GT9966 ||
		   cd->support_vendor_ic_type == GT9916 ||
		   cd->support_vendor_ic_type == GT7986) {
		touch_driver_spi_read_for_9966(tdev, tx_buf, rx_buf, &xfers[0],
					       &spi_msg, addr, len);
	} else {
	...
	while (retry < SPI_READ_RETRY_TIME) {
		...
		if (ret < 0) {
			thp_log_err(tdev->thp_core, "Spi transfer error:%d\n",
				    ret);
			if ((gdix_set_mode.lowpower_mode == 1) &&
			    cd->support_lowpower_no_irq) {
...
static int touch_driver_spi_write(struct thp_device *tdev, unsigned int addr,
				  u8 *data, unsigned int len)
{
	struct spi_device *spi = NULL;
	u8 *tx_buf = NULL;
	...
	if (cd->support_vendor_ic_type == GT9897 ||
	    ...
	    cd->support_vendor_ic_type == GT9916 ||
	    cd->support_vendor_ic_type == GT7986) {
		touch_driver_spi_write_for_9897(tdev, tx_buf, addr, &xfers,
						data, len);
	} else {
		if (addr || (cd->support_vendor_ic_type == GT9896)) {
			tx_buf[index++] =
				SPI_FLAG_WR; /* 0xF1 start read flag */
			tx_buf[index++] = (addr >> MOVE_8BIT) & MASK_8BIT;
			tx_buf[index++] = addr & MASK_8BIT;
	...
	if (ret < 0)
		thp_log_err(tdev->thp_core,
			    "Spi transfer error:%d, addr 0x%x\n", ret, addr);
	thp_bus_unlock(tdev->thp_core);
				    ...
				    unsigned int ack_reg)
{
	...
	for (i = 0; i < CHECK_COMMAND_RETRY; i++) {
		...
		ret = touch_driver_spi_read(tdev, ack_reg, &cmd_ack, 1);
		if (ret < 0) {
			thp_log_err(tdev->thp_core,
				    "%s: failed read cmd ack info, ret %d\n",
				    __func__, ret);
			return -EINVAL;
		}
...
static int touch_driver_switch_cmd_for_9896(struct thp_device *tdev, u8 cmd,
					    u16 data)
{
	int ret;
	int i;
	...
	for (i = 0; i < SEND_COMMAND_RETRY; i++) {
		thp_spi_cmd_lock(tdev->thp_core, SPI_CMD_LOCK_TIMEOUT);
		...
		if (ret < 0) {
			thp_log_err(tdev->thp_core,
				    ...
				    __func__, ret);
			thp_spi_cmd_unlock(tdev->thp_core);
			return -EINVAL;
		}
		ret = touch_driver_get_cmd_ack(
			tdev, private_data->goodix_cmd_addr +
				      GOODIX_CMD_ACK_OFFSET_GT9896);
		thp_spi_cmd_unlock(tdev->thp_core);
		if (ret == CMD_ACK_ERROR) {
			thp_log_err(tdev->thp_core,
				    "%s: cmd ack info is error\n", __func__);
			continue;
		} else {
			break;
		}
...
static int touch_drive_switch_cmd_for_9897(struct thp_device *tdev, u8 cmd,
					   u16 data, int retry)
{
	int ret;
	int i;
	...
	for (i = 0; i < retry; i++) {
		...
		ret = touch_driver_get_cmd_ack_for_9897(
			tdev, private_data->goodix_cmd_addr +
...
static int touch_driver_start_compute_active_doze_time(struct thp_device *tdev)
{
	...
	ret = touch_driver_switch_cmd(tdev, 0x31, ENABLE, SEND_COMMAND_RETRY);
	if (ret < 0) {
					     ...
					     unsigned long long *doze_time)
{
	...
	if (tdev->thp_core->support_vendor_ic_type == GT9916)
		ret = touch_driver_spi_read(
			tdev, REG_GET_ACTIVE_DOZE_TIME_ADDR_GT9916, temp_buf,
			sizeof(temp_buf));
	else if (tdev->thp_core->support_vendor_ic_type == GT9966)
		ret = touch_driver_spi_read(
			tdev, REG_GET_ACTIVE_DOZE_TIME_ADDR_GT9966, temp_buf,
			sizeof(temp_buf));
	else {
	...
	if (ret < 0) {
		thp_log_err(tdev->thp_core,
			    "%s: failed to read active and doze time, ret %d\n",
			    __func__, ret);
		return -EIO;
	}
	/*
	 * small end model; 0~3 bytes of data is active time
	 * 4~7 bytes of data is doze time
	 * the data unit is 100ms
	 */
	active = (unsigned int *)&temp_buf[0];
	doze = (unsigned int *)&temp_buf[4];
	*active_time = ((uint64_t)(*active)) * 100;
	*doze_time = ((uint64_t)(*doze)) * 100;
	ret = touch_driver_start_compute_active_doze_time(tdev);
	if (ret < 0) {
		thp_log_err(tdev->thp_core,
				   ...
				   int retry_count)
{
	...
	if (touch_driver_idle_wakeup(tdev)) {
		thp_log_err(tdev->thp_core,
			    "failed wakeup idle before send command\n");
		return -EINVAL;
	}
	cmd_buf[index++] = cmd;
	...
	while (retry++ < SEND_COMMAND_RETRY) {
		...
		ret = touch_driver_spi_read(
			tdev, private_data->goodix_cmd_addr + CMD_ACK_OFFSET,
		...
		if (cmd_ack != CMD_ACK_OK) {
			ret = -EINVAL;
			thp_log_debug(tdev->thp_core,
				      "%s: command state ack info 0x%x\n",
				      __func__, cmd_ack);
			if (cmd_ack == CMD_ACK_BUF_OVERFLOW ||
				       ...
				       struct thp_core_data *cd)
{
	...
	rc = of_property_read_u32(thp_node, "support_pad_frame_len_max_size",
				  &value);
	if (!rc) {
		cd->support_pad_frame_len_max_size = value;
		thp_log_info(cd, "%s: support_pad_frame_len_max_size %u\n",
			     __func__, value);
	}
...
static int touch_driver_init(struct thp_device *tdev)
{
	...
	cd = tdev->thp_core;
	goodix_node =
		of_get_child_by_name(cd->thp_node, THP_GOODIX_DEV_NODE_NAME);
	...
	rc = thp_parse_timing_config(tdev->thp_core, goodix_node,
				     &tdev->timing_config);
	if (rc)
	...
	if (rc)
		thp_log_err(tdev->thp_core,
			    "%s: feature_ic_config fail, use default\n",
			    __func__);
...
static void touch_driver_init_set(struct thp_device *tdev)
{
	...
	for (i = 0; i < COMMUNREG_RETRY_TIME_GT9897; i++) {
		...
		ret = touch_driver_spi_read(tdev, COMMUNREG_GT9897, temp_buf,
					    sizeof(temp_buf));
		thp_log_err(tdev->thp_core,
			    "%s: read 0XC900, temp_buf[0]= %d\n", __func__,
			    temp_buf[0]);
		if (ret)
			thp_log_err(tdev->thp_core,
				    "%s:  failed read 0XC900, ret %d\n",
				    __func__, ret);
		/* 2-compare */
...
static int touch_driver_communication_check_for_9897(struct thp_device *tdev)
{
	int len;
	int ret;
	int retry;
	...
	for (retry = 0; retry < MODULE_INFO_READ_RETRY; retry++) {
		ret = touch_driver_spi_read(
			tdev, GOODIX_MODULE_INFO_ADDR_GT9897,
			(u8 *)&private_data->module_info_for_gt9897, len);
		/* print 32*3 data (rowsize=32,groupsize=3),0:no ascii */
		print_hex_dump(KERN_INFO,
			       "[I/THP] module info: ", DUMP_PREFIX_NONE, 32, 3,
			       (u8 *)&private_data->module_info_for_gt9897, len,
			       0);
		if (!ret &&
		    !checksum16_cmp(tdev,
				    (u8 *)&private_data->module_info_for_gt9897,
				    len, GOODIX_LE_MODE))
			break;
	...
	if (memcmp(private_data->module_info_for_gt9897.mask_id,
		   GOODIX_MASK_ID_FOR_GT9897, GOODIX_MASK_ID_LEN)) {
		if (memcmp(private_data->module_info_for_gt9897.mask_id,
			   GOODIX_MASK_ID_FOR_GT9897_NOCODE,
			   GOODIX_MASK_ID_LEN)) {
			memcpy(temp_buf,
			       private_data->module_info_for_gt9897.mask_id,
			       GOODIX_MASK_ID_LEN);
			thp_log_err(tdev->thp_core,
				    "invalied mask id %s != %s\n", temp_buf,
				    GOODIX_MASK_ID_FOR_GT9897);
			return -EINVAL;
		}
...
static int touch_driver_communication_check_for_9966(struct thp_device *tdev)
{
	...
	if (tdev->thp_core->support_vendor_ic_type == GT7986) {
		mdelay(GOODIX_DELAY_5MS_7986);
		regval = GOODIX_SPI_SDO_SEL_REG;
		touch_driver_spi_write(tdev, GOODIX_SPI_SDO_SEL_ADDR,
					&regval, 1);
	}

	for (retry = 0; retry < MODULE_INFO_READ_RETRY; retry++) {
		ret = touch_driver_spi_read(
			tdev, GOODIX_MODULE_INFO_ADDR_GT9966,
			(u8 *)&private_data->module_info_for_gt9966, len);

		thp_log_debug(tdev->thp_core, "module_info:%*ph\n", len,
			      (u8 *)&private_data->module_info_for_gt9966);
		if (!ret &&
		    !checksum16_cmp(tdev,
				    (u8 *)&private_data->module_info_for_gt9966,
				    len, GOODIX_LE_MODE))
	...
	memcpy(temp_buf, private_data->module_info_for_gt9966.rom_pid,
	       sizeof(private_data->module_info_for_gt9966.rom_pid));
	...
	memcpy(temp_buf, private_data->module_info_for_gt9966.patch_vid,
	       sizeof(private_data->module_info_for_gt9966.patch_pid));
...
static int touch_driver_communication_check_for_9896(struct thp_device *tdev)
{
	int ret;
	int len;
	int retry;
	...
	for (retry = 0; retry < SPI_ARGS_WRITE_RETRY; retry++) {
		ret = touch_driver_spi_write(
			tdev, GOODIX_SPI_TRANSFER_ARGS_ADDR, &wr_val, 1);
		if (ret < 0) {
			thp_log_err(tdev->thp_core,
		...
		if (ret < 0) {
			thp_log_err(tdev->thp_core,
				    "%s: spi read fail, ret %d\n", __func__,
				    ret);
			return -EINVAL;
		}
		if (wr_val == rd_val)
			break;
		thp_log_info(tdev->thp_core, "%s:wr:0x%X, rd:0x%X", __func__,
			     wr_val, rd_val);
	}
	for (retry = 0; retry < MODULE_INFO_READ_RETRY; retry++) {
		ret = touch_driver_spi_read(
			tdev, GOODIX_MODULE_INFO_ADDR_GT9896,
	...
	if (memcmp(private_data->module_info_for_gt9896.mask_id,
		   GOODIX_MASK_ID_FOR_GT9896, GOODIX_MASK_ID_LEN)) {
		memcpy(temp_buf, private_data->module_info_for_gt9896.mask_id,
		       GOODIX_MASK_ID_LEN);
		thp_log_err(tdev->thp_core, "%s: invalied mask id %s != %s\n",
			    __func__, temp_buf, GOODIX_MASK_ID_FOR_GT9896);
		return -EINVAL;
	}
	thp_log_info(tdev->thp_core, "%s: communication check passed\n",
		     __func__);
...
static int touch_driver_communication_check(struct thp_device *tdev)
{
	...
	for (retry = 0; retry < MODULE_INFO_READ_RETRY; retry++) {
		ret = touch_driver_spi_read(tdev, GOODIX_MODULE_INFO_ADDR,
					    (u8 *)&private_data->module_info,
					    len);
		/* print 32*3 data (rowsize=32,groupsize=3) */
		print_hex_dump(KERN_INFO,
			       "[I/THP] module info: ", DUMP_PREFIX_NONE, 32, 3,
			       (u8 *)&private_data->module_info, len, 0);
	...
	thp_log_info(tdev->thp_core,
		     "hw info: ret %d, checksum 0x%x, retry %d\n", ret,
		     checksum, retry);
	if (retry == MODULE_INFO_READ_RETRY) {
		thp_log_err(tdev->thp_core, "%s:failed read module info\n",
			    __func__);
	...
	if (memcmp(private_data->module_info.mask_id, GOODIX_MASK_ID,
		...
		memcpy(temp_buf, private_data->module_info.mask_id,
		       GOODIX_MASK_ID_LEN);
		thp_log_err(tdev->thp_core, "%s: invalied mask id %s != %s\n",
	...
	temp_buf[GOODIX_MASK_ID_LEN] = '\0';
	thp_log_info(tdev->thp_core, "MASK_ID %s : ver %*ph\n", temp_buf,
		     (u32)sizeof(private_data->module_info.mask_vid),
		     private_data->module_info.mask_vid);
	thp_log_info(tdev->thp_core, "PID %s : ver %*ph\n",
		     private_data->module_info.pid,
		     (u32)sizeof(private_data->module_info.vid),
		     private_data->module_info.vid);
	return 0;
}
...
static int touch_driver_power_on_for_9897(struct thp_device *tdev)
{
	...
	touch_driver_spi_write(tdev, GOODIX_FLASH_CTRLLER_REG1_GT9897, &regval,
			       1);
	regval = GOODIX_FLASH_RW_MODE_GT9897;
	touch_driver_spi_write(tdev, GOODIX_FLASH_CTRLLER_REG2_GT9897, &regval,
			       1);
	mdelay(GOODIX_DELAY_5MS_9897);
	regval = GOODIX_FLASH_EN_MODE_GT9897;
	touch_driver_spi_write(tdev, GOODIX_FLASH_CTRLLER_REG3_GT9897, &regval,
			       1);
	return ret;
}
...
static void touch_driver_timing_work(struct thp_device *tdev)
{
	thp_log_info(tdev->thp_core, "%s:called,do hard reset delay:%d\n",
		     __func__, tdev->timing_config.boot_reset_after_delay_ms);
...
static void touch_driver_show_afe_debug_info(struct thp_device *tdev)
{
	...
	thp_log_info(tdev->thp_core, "debug_buf[20-40] %*ph\n",
		     DEBUG_AFE_DATA_BUF_LEN,
		     debug_buf + DEBUG_AFE_DATA_BUF_OFFSET); /* offset 20 */
	thp_log_info(tdev->thp_core, "debug_buf[40-60] %*ph\n",
		     DEBUG_AFE_DATA_BUF_LEN,
		     debug_buf + DEBUG_AFE_DATA_BUF_OFFSET * 2); /* offset 40 */
	thp_log_info(tdev->thp_core, "debug_buf[60-80] %*ph\n",
		     DEBUG_AFE_DATA_BUF_LEN,
		     debug_buf + DEBUG_AFE_DATA_BUF_OFFSET * 3); /* offset 60 */
	thp_log_info(tdev->thp_core, "debug_buf[80-100] %*ph\n",
		     DEBUG_AFE_DATA_BUF_LEN,
		     debug_buf + DEBUG_AFE_DATA_BUF_OFFSET * 4); /* offset 80 */
...
static void touch_driver_get_afe_addr(struct thp_device *tdev,
				      const u8 *const afe_data_buf,
				      unsigned int len)
{
	struct goodix_device_info *private_data =
		(struct goodix_device_info *)tdev->private_data;
	...
	private_data->goodix_frame_addr =
		(afe_data_buf[GOODIX_FRAME_ADDR_OFFSET_GT9896] << MOVE_8BIT) +
		afe_data_buf[GOODIX_FRAME_ADDR_OFFSET_GT9896 + 1];
	private_data->goodix_frame_len =
		(afe_data_buf[GOODIX_FRAME_LEN_OFFSET_GT9896] << MOVE_8BIT) +
		afe_data_buf[GOODIX_FRAME_LEN_OFFSET_GT9896 + 1];
	private_data->goodix_cmd_addr =
		(afe_data_buf[GOODIX_CMD_ADDR_OFFSET_GT9896] << MOVE_8BIT) +
		afe_data_buf[GOODIX_CMD_ADDR_OFFSET_GT9896 + 1];

	thp_log_info(tdev->thp_core,
		     "%s: frame addr 0x%x, len %d, cmd addr 0x%x\n", __func__,
		     private_data->goodix_frame_addr,
		     private_data->goodix_frame_len,
		     private_data->goodix_cmd_addr);
}
...
static int touch_driver_get_afe_info_for_9896(struct thp_device *tdev)
{
	...
	for (retry = 0; retry < FW_INFO_READ_RETRY; retry++) {
		...
		if (ret) {
			thp_log_err(tdev->thp_core,
				    "%s: failed read afe data length, ret %d\n",
				    __func__, ret);
			goto error;
		}
...
static int touch_driver_get_afe_info_for_9966(struct thp_device *tdev)
{
	...
	if (tdev->thp_core->support_vendor_ic_type == GT9916) {
		private_data->goodix_frame_addr =
			GOODIX_FRAME_ADDR_DEFAULT_GT9916;
		private_data->goodix_frame_len = GOODIX_FRAME_LEN_MAX_GT9916;
	} else {
		private_data->goodix_frame_addr =
			GOODIX_FRAME_ADDR_DEFAULT_GT9966;
		if (tdev->thp_core->support_pad_frame_len_max_size > 0)
			private_data->goodix_frame_len =
				tdev->thp_core->support_pad_frame_len_max_size;
		else
			private_data->goodix_frame_len =
				GOODIX_FRAME_LEN_MAX_GT9966;
	}
...
static int touch_driver_get_afe_info(struct thp_device *tdev)
{
	...
	for (retry = 0; retry < FW_INFO_READ_RETRY; retry++) {
		ret = touch_driver_spi_read(tdev, GOODIX_AFE_INFO_ADDR, buf,
					    sizeof(buf));
		if (ret) {
			thp_log_err(tdev->thp_core,
				    "%s: failed read afe data length, ret %d\n",
		...
		/* data len must be equal or less than GOODIX_MAX_AFE_LEN */
		if ((afe_data_len == 0) ||
		    (afe_data_len > GOODIX_MAX_AFE_LEN) ||
		    (afe_data_len & MASK_1BIT)) {
			thp_log_err(tdev->thp_core,
				    "%s: invalied afe_data_len 0x%x retry\n",
		...
		thp_log_info(tdev->thp_core, "%s: got afe data len %d\n",
			     __func__, afe_data_len);
		ret = touch_driver_spi_read(
			tdev, GOODIX_AFE_INFO_ADDR + INFO_ADDR_BUF_LEN,
			afe_data_buf, afe_data_len);
		if (ret) {
			thp_log_err(tdev->thp_core,
				    "%s: failed read afe data, ret %d\n",
		...
		thp_log_err(
			...
			__func__,
			checksum_32(tdev, afe_data_buf, afe_data_len));
		thp_log_err(tdev->thp_core, "afe_data_buf[0-20] %*ph\n",
			    DEBUG_AFE_DATA_BUF_LEN,
			    afe_data_buf); /* offset 0 */
		...
		thp_log_err(tdev->thp_core, "afe_data_buf[40-60] %*ph\n",
			    DEBUG_AFE_DATA_BUF_LEN,
			    afe_data_buf + DEBUG_AFE_DATA_BUF_OFFSET *
						   2); /* offset 40 */
	...
	if (retry != FW_INFO_READ_RETRY) {
		thp_log_info(tdev->thp_core,
			     "%s: try get useful info from afe data\n",
			     __func__);
		private_data->goodix_frame_addr =
		...
		thp_log_err(tdev->thp_core,
			    "%s: failed get afe info, use default\n", __func__);
		ret = -EINVAL;
	}
...
err_out:
	...
	thp_log_info(tdev->thp_core, "%s: fw status addr 0x%x, len %d\n",
		     __func__, private_data->goodix_fw_status_addr,
		     private_data->goodix_fw_status_len);
	return ret;
}
...
static int touch_driver_chip_detect(struct thp_device *tdev)
{
	...
	if (ret) {
		thp_log_err(tdev->thp_core, "%s: prepare fail: %d\n", __func__,
			    ret);
		goto exit;
	}
	cd = tdev->thp_core;
	...
	if ((cd->send_bt_status_to_fw) && (cd->support_dual_chip_arch) &&
		...
		if (touch_driver_9x_bt_handler(tdev, false))
			thp_log_err(
				tdev->thp_core,
				"power on send stylus3 connect status fail\n");
	}
...
#define FW_LOG_OFFSET_GT9897S \
	(FRAME_HEAD_LEN_GT9897S + FW_ATTR_INFO_LEN_GT9897S)
#define UDFP_DATA_OFFSET_GT9897S                             \
	(FRAME_HEAD_LEN_GT9897S + FW_ATTR_INFO_LEN_GT9897S + \
	 FW_LOG_INFO_LEN_GT9897S)
#define CHECKSUM_SIZE_U16 2
#define CHECKSUM_SIZE_U32 4
#define SCREENOFF_FRAME_LEN_GT9897S                          \
	(FRAME_HEAD_LEN_GT9897S + FW_ATTR_INFO_LEN_GT9897S + \
	 FW_LOG_INFO_LEN_GT9897S + UDFP_DATA_LEN_GT9897S +   \
	 UDFP_TAIL_FRAME_INDEX + CHECKSUM_SIZE_U32)
...
static int touch_driver_get_next_frame_len(struct thp_device *tdev, u8 *buf)
{
	...
	static struct frame_head pre_head = {
		0, 0, 0, FRAME_HEAD_LEN, FRAME_HEAD_LEN, 0, 0, 0, 0};
 ...
 */
static int touch_driver_set_udfp_state(struct thp_device *tdev, u8 state)
{
	int ret;
...
static int touch_driver_set_lowpower_state(struct thp_device *tdev, u8 state)
{
	...
	if ((state == 0) && cd->support_lowpower_no_irq &&
	    private_data->goodix_esd_status) {
	...
	ret = touch_driver_switch_cmd(tdev, CMD_LOWPOWER_SWITCH_GT9897S, state,
				      SEND_COMMAND_MULTIPLE_RETRY);
	if (!ret)
...
static int touch_driver_standby_exit_lowpower(struct thp_device *tdev)
{
	...
	if (private_data->standby_enable_in_lowpower) {
		touch_driver_switch_cmd(tdev, CMD_LOWPOWER_SWITCH_GT9966,
					DISABLE_LP,
					SEND_COMMAND_MULTIPLE_RETRY);
 ...
 * return 0 for success
 */
static int touch_driver_set_touch_gesture_state(struct thp_device *tdev,
						u8 state, u8 touch_click)
{
	...
	if (!state)
		ret = touch_driver_switch_cmd(tdev,
					      CMD_TOUCH_GESTURE_SWITCH_GT9897S,
					      0, SEND_COMMAND_MULTIPLE_RETRY);
	else
		ret = touch_driver_switch_cmd(tdev,
					      CMD_TOUCH_GESTURE_SWITCH_GT9897S,
					      touch_click,
...
static void touch_driver_recover_fw_state(struct thp_device *tdev)
{
	...
	if (gdix_set_mode.touch_gesture)
		touch_driver_set_touch_gesture_state(
			tdev, ENABLE, gdix_set_mode.touch_gesture);
}
						 ...
						 u8 state)
{
	...
	if (state) {
		if (tdev->thp_core->fullaod_support_gesture) {
			ret = touch_driver_switch_cmd(tdev, SLIDE_CMD, ENABLE,
						      SEND_COMMAND_RETRY);
			if (tdev->thp_core->easy_wakeup_info.sleep_mode !=
			    TS_GESTURE_MODE)
				ret = touch_driver_set_touch_gesture_state(
					tdev, ENABLE, CMD_GESTURE_DOUBLE);
		} else {
			ret = touch_driver_switch_cmd(
				tdev, CMD_AOD_EVENT_SWITCH_GT9897S, DISABLE,
				SEND_COMMAND_MULTIPLE_RETRY);
		}
	} else {
		ret = touch_driver_switch_cmd(tdev, SLIDE_CMD, DISABLE,
					      SEND_COMMAND_RETRY);
		if (tdev->thp_core->easy_wakeup_info.sleep_mode !=
		    TS_GESTURE_MODE)
			ret = touch_driver_set_touch_gesture_state(
				tdev, DISABLE, CMD_GESTURE_DOUBLE);
		ret = touch_driver_switch_cmd(tdev,
					      CMD_AOD_EVENT_SWITCH_GT9897S,
					      ENABLE,
					      SEND_COMMAND_MULTIPLE_RETRY);
	}
}

static int goodix_touch_handler(struct thp_device *tdev,
				struct thp_udfp_data *udfp_data)
{
	...
	if (touch_num == 0) { /* release all fingers */
		if (private_data->tui_finger_last_status !=
		    TP_EVENT_FINGER_UP) {
			udfp_data->tpud_data.udfp_event = TP_EVENT_FINGER_UP;
			private_data->tui_finger_last_status =
				TP_EVENT_FINGER_UP;
		}
		return 0;
	} else {
		if (private_data->tui_finger_last_status !=
		    TP_EVENT_FINGER_DOWN) {
	...
	for (i = 0; i < touch_num; i++) {
		if (!tdev->thp_core->standby_mode) {
			...
			udfp_data->tpud_data.tp_x =
				le16_to_cpup((__le16 *)(coor_data + 2));
			udfp_data->tpud_data.tp_y =
				le16_to_cpup((__le16 *)(coor_data + 4));
		} else {
			// for standby
			...
			udfp_data->coor_data[i].y =
				le16_to_cpup((__le16 *)(coor_data + 4));
			udfp_data->coor_data[i].tracking_id =
				(le16_to_cpup((__le16 *)(coor_data)) >> 4) &
				0x0F;
		}
		coor_data += 8;
...
static int parse_event_info(struct thp_device *tdev, const u8 *read_buff,
			    unsigned int len, struct thp_udfp_data *udfp_data)
{
	unsigned int tpfp_event[TP_FP_VALID_EVENT_NUM] = {
		TP_EVENT_FINGER_DOWN, TP_EVENT_FINGER_UP, TP_EVENT_HOVER_DOWN,
		TP_EVENT_HOVER_UP, TP_EVENT_FINGER_UP_QDB};
	int i;
	struct goodix_device_info *private_data =
		(struct goodix_device_info *)tdev->private_data;
	...
	udfp_data->tpud_data.version =
		read_buff[0] + (read_buff[1] << MOVE_8BIT) +
		(read_buff[2] << MOVE_16BIT) + (read_buff[3] << MOVE_24BIT);
	/* read_buff [4:7] = udfp_event */
	udfp_data->tpud_data.udfp_event =
		read_buff[4] + (read_buff[5] << MOVE_8BIT) +
	...
	udfp_data->tpud_data.tp_x =
		read_buff[32] + (read_buff[33] << MOVE_8BIT) +
		(read_buff[34] << MOVE_16BIT) + (read_buff[35] << MOVE_24BIT);
	/* read_buff [36:39]  = tp_y */
	udfp_data->tpud_data.tp_y =
		read_buff[36] + (read_buff[37] << MOVE_8BIT) +
	...
	udfp_data->aod_event = read_buff[56] + (read_buff[57] << MOVE_8BIT) +
			       (read_buff[58] << MOVE_16BIT) +
			       (read_buff[59] << MOVE_24BIT);
	/* read_buff [60:63]  = key_event */
	udfp_data->key_event = read_buff[60] + (read_buff[61] << MOVE_8BIT) +
			       (read_buff[62] << MOVE_16BIT) +
			       (read_buff[63] << MOVE_24BIT);
	/* read_buff [64:67]  = slide_event */
	udfp_data->slide_event = read_buff[64] + (read_buff[65] << MOVE_8BIT) +
				 (read_buff[66] << MOVE_16BIT) +
				 (read_buff[67] << MOVE_24BIT);
	thp_log_info(
		tdev->thp_core,
		"%s:version:%x, udfp_event:%d, aod:%u, key:%u slide_event:%u\n",
		__func__, udfp_data->tpud_data.version,
		udfp_data->tpud_data.udfp_event, udfp_data->aod_event,
		udfp_data->key_event, udfp_data->slide_event);
	...
	/*
	 * large multi main tp need report slide start and end position.
	 * udfp_data->tpud_data.tp_x: bit 31 for if support, support(1), no support(0).
	 * udfp_data->tpud_data.tp_x: bit 0-15 for start_x, bit 16-30 for end_x.
	 * udfp_data->tpud_data.tp_y: bit 0-15 for start_y, bit 16-31 for end_y.
	 */
	udfp_data->slide_position.support_report_flag =
		(udfp_data->tpud_data.tp_x & FULLAOD_SLIDE_POSITION_REPORT);
	if (udfp_data->slide_position.support_report_flag) {
		udfp_data->slide_position.start_x = (udfp_data->tpud_data.tp_x & 0xFFFF);
		udfp_data->slide_position.end_x =
			((udfp_data->tpud_data.tp_x >> MOVE_16BIT) & 0x7FFF);
		udfp_data->slide_position.start_y = (udfp_data->tpud_data.tp_y & 0xFFFF);
		udfp_data->slide_position.end_y =
			((udfp_data->tpud_data.tp_y >> MOVE_16BIT) & 0xFFFF);
	}
	return NO_ERR;
}
...
{
	u8 read_buff[SCREENOFF_FRAME_LEN_GT9897S] = {0};
	u8 j;
	int ret = 0;
	struct debug_info_struct *debug_info_struct = NULL;

	if (tdev->thp_core->support_vendor_ic_type == GT9916) {
		ret = touch_driver_spi_read(
			tdev, GOODIX_FRAME_NORMALIZATION_ADDR_DEFAULT_GT9916,
			read_buff, SCREENOFF_FRAME_LEN_GT9897S);
	} else if (tdev->thp_core->support_vendor_ic_type == GT9966) {
		ret = touch_driver_spi_read(
			tdev, GOODIX_FRAME_NORMALIZATION_ADDR_DEFAULT_GT9966,
			read_buff, SCREENOFF_FRAME_LEN_GT9897S);
	}

	if (ret) {
		thp_log_err(tdev->thp_core,
			    "%s: failed read read_buff, ret %d\n", __func__,
			    ret);
		return;
	}
	debug_info_struct = (struct debug_info_struct *)read_buff;
				       ...
				       struct thp_udfp_data *udfp_data)
{
	...
	if (cd->support_lowpower_no_irq && private_data->goodix_esd_status) {
		thp_log_err(tdev->thp_core,
			    "%s: No processing in lowpower state\n", __func__);
		return -EFAULT;
	}
	/*
	* 0x1007F(GOODIX_DEBUG_INFO_ADDR) reads a byte. If the content is 0XAA,
        * it means normalized printing of information is supported.
	* If not, it means the old log firmware.
	*/
	ret = touch_driver_spi_read(tdev, GOODIX_DEBUG_INFO_ADDR,
				    &debug_info_flag, sizeof(debug_info_flag));
	ret = touch_driver_spi_read(tdev, private_data->goodix_frame_addr,
				    frame_buf, SCREENOFF_FRAME_LEN_GT9897S);
	if (debug_info_flag == 0xAA) {
		prase_touch_information_normalize_debug_info(tdev);
	} else {
		for (i = GOODIX_DEBUG_LEN_START; i < GOODIX_DEBUG_LEN_END;
		     i = i + 8)
			thp_log_info(
				tdev->thp_core,
				"tp_info %d~%d: %x, %x, %x, %x, %x, %x, %x, %x\n",
				i - GOODIX_DEBUG_LEN_START,
				i - GOODIX_DEBUG_LEN_START + 7, frame_buf[i],
				frame_buf[i + 1], frame_buf[i + 2],
				frame_buf[i + 3], frame_buf[i + 4],
				frame_buf[i + 5], frame_buf[i + 6],
				frame_buf[i + 7]);
	}
	...
	if ((ret < 0) || le32_data_check(frame_buf, sizeof(frame_buf))) {
		thp_log_err(tdev->thp_core,
			    "failed get valied frame data, %d,len %d", ret,
			    sizeof(frame_buf));
		thp_log_info(tdev->thp_core, "fw_log:%*ph",
			     FRAME_HEAD_LEN_GT9897S, frame_buf);
		tdev->thp_core->event_anomaly_count++;
		if (tdev->thp_core->event_anomaly_count >
		    EVENT_ANOMALY_MAX_COUNT) {
			thp_log_err(tdev->thp_core,
				    "event_anomaly_count++ %u, self recovery\n",
				    tdev->thp_core->event_anomaly_count);
	...
	tail_index =
		(frame_buf[SCREENOFF_FRAME_LEN_GT9897S - CHECKSUM_SIZE_U32 - 1]
		 << 8) |
	...
	if (!(head_info->data_en & UDFP_PACK_EN)) {
		thp_log_err(tdev->thp_core, "no udfp package 0x%x",
			    head_info->data_en);
	...
	thp_log_info(tdev->thp_core, "udfp:%*ph",
		     UDFP_DATA_LEN_GT9897S - UDFP_DATA_LEN_MIN,
				  ...
				  unsigned int len)
{
	...
	if (((tdev->thp_core->support_vendor_ic_type == GT9897) ||
		...
		ret = touch_driver_spi_read(
			tdev, private_data->goodix_frame_addr, buf, frame_len);
		frame_len = touch_driver_get_next_frame_len(tdev, buf);
		return ret;
	}

	return touch_driver_spi_read(tdev, private_data->goodix_frame_addr, buf,
				     len);
}
				       ...
				       unsigned int *gesture_wakeup_value)
{
	...
	if ((tdev->thp_core->support_vendor_ic_type == GT9897 ||
	     tdev->thp_core->support_vendor_ic_type == GT7986) &&
	    (tdev->thp_core->support_dual_chip_arch))
		retval = touch_driver_gesture_event_for_gt9897_dual(
			tdev, gesture_wakeup_value);
					  ...
					  u8 switch_value)
{
	...
	if (switch_value) {
		result = touch_driver_switch_cmd(
			tdev, CMD_TUI_GT9966, ENABLE_TUI, SEND_COMMAND_RETRY);
		if (result)
			thp_log_err(tdev->thp_core, "failed enable tui mode\n");
	} else {
		result = touch_driver_switch_cmd(
			tdev, CMD_TUI_GT9966, DISABLE_TUI, SEND_COMMAND_RETRY);
		if (result)
			thp_log_err(tdev->thp_core,
				    "failed disable tui mode\n");
	}
...
static int touch_driver_wakeup_gesture_enable_switch(struct thp_device *tdev,
						     u8 switch_value)
{
	int retval = NO_ERR;
	int result;
	...
	if (switch_value) {
		result = touch_driver_switch_cmd(
			tdev, tdev->thp_core->cmd_gesture_mode, FEATURE_ENABLE,
			SEND_COMMAND_RETRY);
		if (result)
			thp_log_err(tdev->thp_core,
				    "failed enable gesture mode\n");
		...
		retval = touch_driver_switch_cmd(
			tdev, tdev->thp_core->cmd_gesture_mode, FEATURE_DISABLE,
			SEND_COMMAND_RETRY);
		if (retval) {
			thp_log_err(tdev->thp_core,
				    "failed disable gesture mode\n");
		} else {
...
static int
touch_driver_wakeup_gesture_enable_switch_9897(struct thp_device *tdev,
					       u8 switch_value)
{
	int retval;
	int result;
	...
	if (switch_value) {
		retval = touch_driver_switch_cmd(tdev, CMD_SCREEN_ON_OFF_9897,
						 1, SEND_COMMAND_RETRY);
		msleep(20); /* delay time 20ms */
		result = touch_driver_switch_cmd(
			tdev, tdev->thp_core->cmd_gesture_mode,
	...
	retval = touch_driver_switch_cmd(tdev, tdev->thp_core->cmd_gesture_mode,
					 FEATURE_DISABLE, SEND_COMMAND_RETRY);
	if (retval) {
				      ...
				      unsigned int *gesture_wakeup_value)
{
	...
	if (!(private_data->work_status & IC_STATUS_GESTURE)) {
		thp_log_info(tdev->thp_core,
			     "%s:please enable gesture mode first\n", __func__);
		retval = -EINVAL;
		goto err_out;
	}
	...
	if (retval) {
		thp_log_err(tdev->thp_core,
			    "%s: failed read gesture head info, ret %d\n",
			    __func__, retval);
		return -EINVAL;
	}

	thp_log_info(tdev->thp_core, "gesute_data:%*ph\n",
		     (u32)sizeof(gesture_event_head), gesture_event_head);
	if (gesture_event_head[0] != IRQ_EVENT_TYPE_GESTURE) {
		thp_log_err(tdev->thp_core,
			    "%s: not gesture irq event, event_type 0x%x\n",
	...
	gesture_type =
		(gesture_event_head[1] << MOVE_8BIT) + gesture_event_head[2];
	...
	/* clean sync flag */
	if (touch_driver_spi_write(tdev, private_data->goodix_frame_addr,
				   &sync_flag, 1))
		thp_log_err(tdev->thp_core, "%s: clean sync flag fail",
			    __func__);
	return retval;
...
err_out:
	...
	if (result || retval)
		...
		thp_log_info(tdev->thp_core,
			     "success resend gesture command\n");
	return -EINVAL;
}
					   ...
					   unsigned int *gesture_wakeup_value)
{
	...
	if (!(private_data->work_status & IC_STATUS_GESTURE)) {
		thp_log_info(tdev->thp_core,
			     "please enable gesture mode first\n");
		return -EINVAL;
	}
	msleep(WAIT_FOR_SPI_BUS_RESUMED_DELAY);
	/* wait spi bus resume */
					   ...
					   unsigned int *gesture_wakeup_value)
{
	...
	msleep(WAIT_FOR_SPI_BUS_RESUMED_DELAY);
	if (tdev->thp_core->support_vendor_ic_type == GT7986)
		private_data->goodix_touch_addr =
			GOODIX_GESTURE_ADDR_DEFAULT_GT7986;
	/* wait spi bus resume */
	for (i = 0; i < GESTURE_EVENT_RETRY_TIME; i++) {
		retval = touch_driver_spi_read(tdev,
					       private_data->goodix_touch_addr,
		...
		if (retval == 0)
			break;
		thp_log_err(tdev->thp_core,
			    "spi not work normal, ret %d retry\n", retval);
		msleep(WAIT_FOR_SPI_BUS_READ_DELAY);
	}
	if (retval) {
		thp_log_err(tdev->thp_core,
			    "failed read gesture head info, ret %d\n", retval);
		return -EINVAL;
	}
	...
	if (gesture_event_head[0] != IRQ_EVENT_TYPE_GESTURE_GT9897_DUAL) {
		thp_log_err(tdev->thp_core,
			    "not gesture irq event, event_type 0x%x\n",
			    gesture_event_head[0]);
		retval = -EINVAL;
		goto err_out;
	}
	...
	/* clean sync flag */
	if (touch_driver_spi_write(tdev, private_data->goodix_touch_addr,
				   &sync_flag, 1))
...
err_out:
	...
	if (touch_driver_spi_write(tdev, private_data->goodix_touch_addr,
				   &sync_flag, 1))
		thp_log_err(tdev->thp_core,
			    "clean sync flag before resend gesture cmd fail\n");
	/* resend gesture command */
	result = touch_driver_switch_cmd(tdev, CMD_SCREEN_ON_OFF,
					 FEATURE_ENABLE, SEND_COMMAND_RETRY);
	if (result)
...
static int touch_driver_resume_shb(struct thp_device *tdev)
{
	...
	if (ret) {
		thp_log_err(tdev->thp_core,
			    "%s:touch_driver_switch_cmd send err, ret = %d\n",
			    __func__, ret);
		return ret;
	}
	thp_log_info(tdev->thp_core,
		     "%s:touch_driver_switch_cmd send: switch int ap\n",
		     __func__);
	return ret;
}
#endif
...
static int touch_driver_resume(struct thp_device *tdev)
{
	...
	if (private_data->need_power_off == NEED_WORK_IN_SUSPEND) {
#ifdef CONFIG_HONOR_SHB_THP
...
static int touch_driver_after_resume(struct thp_device *tdev)
{
	...
	if (!private_data->thp_udfp_status && !is_pt_test_mode(tdev)) {
		...
		if (ret)
			thp_log_err(tdev->thp_core,
				    "failed send CMD_TOUCH_REPORT mode\n");
	}

	return ret;
}

static int send_stylus3_status_to_fw(struct thp_device *tdev, u8 *bt_status_buf,
				     u8 check_status_data)
{
	int i;
	int ret;
	int connect_addr;
	...
	if (tdev->thp_core->support_vendor_ic_type == GT7986) {
		...
		connect_check_addr = BT_CONNECT_STATUS_CHECK_ADDR_GT7986;
	} else {
		connect_addr = BT_CONNECT_STATUS_ADDR;
		connect_check_addr = BT_CONNECT_STATUS_CHECK_ADDR;
	}
	for (i = 0; i < BT_CONNECT_CMD_RETRY; i++) {
		ret = touch_driver_spi_write(tdev, connect_addr, bt_status_buf,
					     BT_CONNECT_STATUS_BUF_LEN);
		if (ret)
			continue;
				      ...
				      bool delay_enable)
{
	...
	u8 bt_connect_value;
	u8 bt_connect_buf[BT_CONNECT_STATUS_BUF_LEN] = {0,
							0,
							BT_CONNECT_BUF2,
							BT_CONNECT_BUF3,
							BT_CONNECT_BUF4,
							BT_CONNECT_BUF5,
							0};
	u8 bt_disconnect_buf[BT_CONNECT_STATUS_BUF_LEN] = {0,
							   0,
							   BT_DISCONNECT_BUF2,
							   BT_DISCONNECT_BUF3,
							   0,
	...
	ret = send_stylus3_status_to_fw(tdev, bt_status_buf_addr,
					bt_connect_value);
	if (ret) {
		thp_log_err(tdev->thp_core,
			    "%s: send stylus3 status to fw fail\n", __func__);
		return ret;
	}
...
static int touch_driver_set_ap_state(struct thp_device *tdev)
{
	...
	if (tdev->thp_core->easy_wakeup_info.sleep_mode == TS_GESTURE_MODE) {
		ret = touch_driver_set_touch_gesture_state(tdev, ENABLE,
							   CMD_GESTURE_DOUBLE);
		if (ret)
			thp_log_err(tdev->thp_core,
				    "failed to set_touch_gesture\n");
	}
...
static int touch_driver_suspend_ap(struct thp_device *tdev)
{
	...
	if (tdev->thp_core->use_ap_gesture) {
		if ((private_data->stylus_gesture_status_only &&
			...
			ret = touch_driver_switch_cmd(
				tdev, CMD_LOWPOWER_SWITCH_GT9966, ENABLE_LP,
				SEND_COMMAND_MULTIPLE_RETRY);
			if (private_data->stylus_gesture_status_only)
				private_data->stylus_gesture_status_lowpower =
					true;
			else
	...
	if (tdev->thp_core->easy_wakeup_info.sleep_mode == TS_GESTURE_MODE) {
		...
		if ((tdev->thp_core->support_vendor_ic_type == GT9897 ||
			...
			ret = touch_driver_wakeup_gesture_enable_switch(
				tdev, FEATURE_ENABLE);
		if (ret)
			thp_log_err(tdev->thp_core,
				    "failed to wakeup gesture enable\n");
		mutex_lock(&tdev->thp_core->thp_wrong_touch_lock);
		tdev->thp_core->easy_wakeup_info.off_motion_on = true;
		mutex_unlock(&tdev->thp_core->thp_wrong_touch_lock);
	}
	...
	if (tdev->thp_core->support_vendor_ic_type == GT7986) {
		if (!(tdev->thp_core->easy_wakeup_info.easy_wakeup_gesture))
			ret = touch_driver_wakeup_gesture_enable_switch(
				tdev, FEATURE_DISABLE);
	}
	...
	if (tdev->thp_core->tsa_event_to_udfp &&
	    private_data->thp_udfp_status) {
		mdelay(10);
...
static int touch_driver_suspend_shb(struct thp_device *tdev)
{
	...
	thp_log_info(tdev->thp_core,
		     "%s:touch_driver_switch_cmd send: switch int shb\n",
		     __func__);
	return ret;
}
...
static int touch_driver_suspend(struct thp_device *tdev)
{
	...
	private_data->stylus_gesture_status_only =
		(!private_data->thp_udfp_status &&
		 ...
		 !cd->easy_wakeup_info.sleep_mode && !cd->standby_enable &&
		 private_data->stylus_gesture_status);
	private_data->standby_enable_only =
		(!private_data->thp_udfp_status &&
		 !private_data->aod_touch_status &&
		 !cd->easy_wakeup_info.sleep_mode &&
		 !private_data->stylus_gesture_status && cd->standby_enable);
	if ((cd->support_vendor_ic_type == GT9897 ||
	     cd->support_vendor_ic_type == GT7986) &&
	...
	private_data->force_power_off_state =
		cd->force_power_off ||
		cd->easy_wakeup_info.fold_force_power_off;
	thp_log_info(
		tdev->thp_core,
		"%s:gesture_status:%d,finger_status:%u, standby_enable:%u\n",
		__func__, cd->easy_wakeup_info.sleep_mode,
		private_data->thp_udfp_status, cd->standby_enable);
	thp_log_info(
		tdev->thp_core,
	...
	thp_log_info(
		tdev->thp_core,
		"%s:stylus_status:%u,aod_touch_status:%u, talk_mode_flag:%u\n",
		__func__, private_data->stylus_gesture_status,
		cd->aod_touch_status, cd->talk_mode_flag);
	thp_log_info(tdev->thp_core,
		     "%s:force_power_off:%u, fold_force_power_off:%u\n",
		     __func__, private_data->force_power_off_state,
		     cd->easy_wakeup_info.fold_force_power_off);
	stylus3_status = atomic_read(&tdev->thp_core->last_stylus3_status);
	if (cd->support_vendor_ic_type == GT7986) {
		private_data->stylus_gesture_status =
			((stylus3_status > 0) &&
			 private_data->stylus_gesture_status);
		thp_log_info(tdev->thp_core, "%s:stylus_status:%u\n", __func__,
			     private_data->stylus_gesture_status);
	}
	if (cd->support_gesture_mode) {
		if ((stylus3_status > 0) && (cd->stylus_gesture_status == 0)) {
			ret = touch_driver_switch_cmd(tdev, PEN_SWITCH,
						      STYLUS_CLOSE,
						      SEND_COMMAND_RETRY);
			if (ret)
				thp_log_err(tdev->thp_core,
					    "%s:close pen failed,ret = %d\n",
					    __func__, ret);
		}
	}
	if (is_pt_test_mode(tdev)) {
		...
		if (cd->support_vendor_ic_type == GT9966) {
			ret = touch_driver_switch_cmd(tdev, CMD_PT_OUT_IDLE,
						      PT_MODE,
						      SEND_COMMAND_RETRY);
			if (!ret)
				msleep(30);
		...
		if (cd->support_vendor_ic_type == GT9897 ||
		    ...
		    cd->support_vendor_ic_type == GT9916 ||
		    cd->support_vendor_ic_type == GT7986)
			ret = touch_driver_switch_cmd(tdev, CMD_PT_MODE_GT9897,
						      PT_MODE,
						      SEND_COMMAND_RETRY);
		else
			ret = touch_driver_switch_cmd(
				tdev, CMD_PT_MODE, PT_MODE, SEND_COMMAND_RETRY);
		if (ret)
			thp_log_err(tdev->thp_core, "failed enable PT mode\n");
	} else if ((private_data->thp_udfp_status ||
		...
		if (cd->support_shb_thp) {
			...
			private_data->need_power_off = NEED_WORK_IN_SUSPEND;
			ret = touch_driver_suspend_ap(tdev);
			if (ret < 0)
				thp_log_err(
					tdev->thp_core,
					"%s: touch_driver_suspend_ap fail :%d\n",
					__func__, ret);
		...
		private_data->need_power_off = NO_NEED_WORK_IN_SUSPEND;
...
static void touch_driver_get_oem_info(struct thp_device *tdev, const char *buff)
{
	struct thp_core_data *cd = tdev->thp_core;
	...
	if (tdev->thp_core->support_oem_info == THP_OEM_INFO_LCD_EFFECT_TYPE) {
		...
		ret = snprintf(cd->oem_info_data, OEM_INFO_DATA_LENGTH, "0x%x",
			       lcd_effect_flag);
		if (ret < 0)
				       ...
				       unsigned int len)
{
	...
	cd = tdev->thp_core;
	if (cd->support_vendor_ic_type == GT9896)
		return touch_driver_get_project_id_for_989x(
			tdev, GOODIX_PROJECT_ID_ADDR_GT9896, buf, len);
	else if (cd->support_vendor_ic_type == GT9897)
		return touch_driver_get_project_id_for_989x(
			tdev, GOODIX_PROJECT_ID_ADDR_GT9897, buf, len);
	else if (cd->support_vendor_ic_type == GT9966 ||
		 cd->support_vendor_ic_type == GT7986 ||
		 cd->support_vendor_ic_type == GT9916)
		return touch_driver_get_project_id_for_989x(
			tdev, GOODIX_PROJECT_ID_ADDR_GT9966, buf, len);
...
static int touch_driver_suspend_connect_pen(struct thp_device *tdev)
{
	...
	if (private_data->stylus_gesture_status_lowpower) {
		ret = touch_driver_switch_cmd(tdev, CMD_LOWPOWER_SWITCH_GT9966,
					      DISABLE_LP,
					      SEND_COMMAND_MULTIPLE_RETRY);
		if (ret) {
			thp_log_err(tdev->thp_core,
				    "%s:disable lowpower failed\n", __func__);
			return ret;
		}
		private_data->stylus_gesture_status_lowpower = false;
	}
	ret = touch_driver_switch_cmd(tdev, PEN_SWITCH, STYLUS_OPEN,
				      SEND_COMMAND_RETRY);
	return ret;
}
...
static int touch_driver_suspend_disconnect_pen(struct thp_device *tdev)
{
	...
	ret = touch_driver_switch_cmd(tdev, PEN_SWITCH, STYLUS_CLOSE,
				      SEND_COMMAND_RETRY);
	if (ret)
...
int touch_driver_spi_active(struct thp_device *tdev)
{
	...
	if (ret) {
		thp_log_err(tdev->thp_core,
			    "failed set spi speed to %dHz, ret %d\n",
			    IDLE_SPI_SPEED, ret);
		return ret;
	}
...
static int touch_driver_idle_wakeup(struct thp_device *tdev)
{
	...
	if (!private_data->goodix_fw_status_addr ||
	    !private_data->goodix_fw_status_len ||
	    private_data->goodix_fw_status_len > MAX_FW_STATUS_DATA_LEN) {
		thp_log_err(tdev->thp_core,
			    "%s: invalid fw status reg, length: %d\n", __func__,
			    private_data->goodix_fw_status_len);
		return 0;
	}
	...
	if (ret) {
		thp_log_err(tdev->thp_core,
			    "failed read fw status info data, ret %d\n", ret);
		return -EIO;
	}

	if (!checksum_16(tdev, fw_status_data,
			 private_data->goodix_fw_status_len) &&
	...
	for (i = 0; i < SPI_WAKEUP_RETRY; i++) {
		...
		if (ret) {
			thp_log_debug(tdev->thp_core,
				      "failed write spi active flag, ret %d\n",
				      ret);
			continue;
		}
		/* recheck spi state */
		...
		thp_log_debug(tdev->thp_core, "fw status data:%*ph\n",
			      ...
			      fw_status_data);
		thp_log_debug(tdev->thp_core,
			      "failed wakeup form idle retry %d\n", i);
	}
	return -EIO;
}
...
static int touch_driver_prepare(struct thp_device *tdev)
{
	...
	if (private_data->goodix_spi_tx_buf == NULL) {
		private_data->goodix_spi_tx_buf =
			kzalloc(READ_CMD_BUF_LEN, GFP_KERNEL);
		if (private_data->goodix_spi_tx_buf == NULL) {
			thp_log_err(tdev->thp_core,
				    "%s: goodix_spi_tx_buf null\n", __func__);
			return -ENOMEM;
		}
	}
	return NO_ERR;
...
struct thp_device *goodix_driver_module_alloc(struct thp_core_data *cd)
{
	struct thp_device *dev = NULL;
	struct goodix_device_info *device_info = NULL;
	int len;
...
int goodix_driver_module_init(struct thp_core_data *cd)
{
	...
	dev = goodix_driver_module_alloc(cd);
	if (!dev)
		return -ENOMEM;
