...
#include "honor_thp.h"
#if (defined CONFIG_HONOR_THP_WRAPPER)
...
static struct thp_core_data *misc_dev_get_core_data(struct miscdevice *dev)
{
	...
	if (g_thp_core_sub && g_thp_core_sub->thp_mt_wrapper_misc_device) {
		if (dev->name ==
		    g_thp_core_sub->thp_mt_wrapper_misc_device->name)
...
void thp_inputkey_report(struct thp_core_data *cd,
			 unsigned int gesture_wakeup_value)
{
	input_report_key(cd->thp_mt_wrapper->input_dev, gesture_wakeup_value,
			 1);
...
#if (IS_ENABLED(CONFIG_HONOR_THP_QCOM) || IS_ENABLED(CONFIG_HONOR_THP_MTK))
void thp_aod_click_report(struct thp_core_data *cd,
...
void thp_udfp_event_to_aod(struct thp_core_data *cd,
			   struct thp_udfp_data udfp_data)
{
	struct input_dev *input_dev = cd->thp_mt_wrapper->input_dev;
	...
	if (udfp_data.tpud_data.udfp_event == TP_EVENT_FINGER_DOWN) {
		...
		input_sync(input_dev);
		thp_lcd_status_notifier(cd, TS_EVENT_FINGER_DOWN);
	}
...
void thp_tui_report_to_input(struct thp_core_data *cd,
			     struct thp_udfp_data udfp_data)
{
	unsigned int x;
	unsigned int y;
	...
	for (int i = 0; i < INPUT_MT_WRAPPER_MAX_FINGERS; i++) {
		if (udfp_data.coor_data[i].valid) {
			x = udfp_data.coor_data[i].x * cd->res_scale;
			y = udfp_data.coor_data[i].y * cd->res_scale;
			tracking_id = udfp_data.coor_data[i].tracking_id;
			input_report_abs(input_dev, ABS_MT_POSITION_X, x);
			input_report_abs(input_dev, ABS_MT_POSITION_Y, y);
			input_report_abs(input_dev, ABS_MT_PRESSURE, 500);
			input_report_abs(input_dev, ABS_MT_TRACKING_ID,
					 tracking_id);
			input_mt_sync(input_dev);
			thp_log_info(cd, "%s ->done, %s, (%u, %u) id %d\n",
				     __func__, input_dev->name,
				     x / cd->res_scale, y / cd->res_scale,
				     tracking_id);
		}
	}
	...
	if (udfp_data.tpud_data.udfp_event == TP_EVENT_FINGER_DOWN) {
		...
		thp_log_info(cd, "%s: down\n", __func__);
	} else if (udfp_data.tpud_data.udfp_event == TP_EVENT_FINGER_UP) {
		input_report_key(input_dev, BTN_TOUCH, 0);
		thp_log_info(cd, "%s: up\n", __func__);
	}
	input_sync(input_dev);
}
#endif

void thp_slide_event_report(struct thp_core_data *cd,
			    struct thp_udfp_data udfp_data)
{
	struct input_dev *input_dev = cd->thp_mt_wrapper->input_dev;
	...
	if (udfp_data.slide_position.support_report_flag) {
		input_report_abs(input_dev, ABS_MT_POSITION_X, udfp_data.slide_position.start_x);
		input_report_abs(input_dev, ABS_MT_POSITION_Y, udfp_data.slide_position.start_y);
		input_report_abs(input_dev, ABS_MT_PRESSURE, 500);
		input_mt_sync(input_dev);
		input_report_key(input_dev, KEY_FULLAOD_SLIDE_EVENT, 1);
		input_sync(input_dev);

		input_report_abs(input_dev, ABS_MT_POSITION_X, udfp_data.slide_position.end_x);
		input_report_abs(input_dev, ABS_MT_POSITION_Y, udfp_data.slide_position.end_y);
		input_report_abs(input_dev, ABS_MT_PRESSURE, 500);
		input_mt_sync(input_dev);
		input_report_key(input_dev, KEY_FULLAOD_SLIDE_EVENT, 0);
		input_sync(input_dev);
		thp_log_info(cd, "%s ->done, start:(%u, %u), end:(%u, %u)\n",
			     __func__, udfp_data.slide_position.start_x,
			     udfp_data.slide_position.start_y,
...
void thp_input_pen_report(struct thp_core_data *cd,
			  unsigned int pen_event_value)
{
...
int thp_mt_wrapper_ioctl_get_events(struct thp_core_data *cd,
				    unsigned long event)
{
	int t;
...
static void thp_adjustment_coordinate(struct thp_core_data *cd)
{
	...
	delta_time =
		((cd->report_cur_time.tv_sec - cd->report_pre_time.tv_sec) *
		 SECOND) +
...
static void thp_coordinate_report(struct thp_core_data *cd,
				  ...
				  struct thp_mt_wrapper_ioctl_touch_data data)
{
	...
	for (i = 0; i < INPUT_MT_WRAPPER_MAX_FINGERS; i++) {
		...
		if (data.touch[i].valid != 0) {
			...
			if (data.touch[i].expand_flag &
			    THP_EXPAND_ENABLED_FLAG) {
				data.touch[i].expand_flag &=
					(~THP_EXPAND_ENABLED_FLAG);
				input_dev->absinfo[ABS_TP_EXPAND_FLAG].value =
					~data.touch[i].expand_flag;
				input_report_abs(input_dev, ABS_TP_EXPAND_FLAG,
						 data.touch[i].expand_flag);
			}
			if (!cd->support_b_protocol)
				input_mt_sync(input_dev);
	...
	/* Enable touch_boost when there is a finger touch */
	if (data.t_num > 0 && cd->irq_boost_touch)
		boost_touch_event();
	/* BTN_TOUCH DOWN */
	if (data.t_num > 0)
		input_report_key(input_dev, BTN_TOUCH, 1);
...
static long thp_mt_wrapper_ioctl_set_coordinate(struct thp_core_data *cd,
						unsigned long arg)
{
	...
	if (!input_dev) {
		thp_log_err(cd, "%s:input_dev is null\n", __func__);
		return -EINVAL;
	}
	...
	thp_coordinate_report(cd, input_dev, data);
...
static int thp_mt_wrapper_ioctl_read_status(struct thp_core_data *cd,
					    unsigned long arg)
{
...
static int thp_mt_ioctl_read_input_config(struct thp_core_data *cd,
					  unsigned long arg)
{
...
static int thp_mt_wrapper_ioctl_read_scene_info(struct thp_core_data *cd,
						unsigned long arg)
{
	...
	thp_log_info(cd, "%s:%d,%d,%d\n", __func__, scene_info->type,
		     scene_info->status, scene_info->parameter);
...
static int thp_mt_wrapper_ioctl_get_window_info(struct thp_core_data *cd,
						unsigned long arg)
{
	...
	thp_log_info(cd, "%s:x0=%d,y0=%d,x1=%d,y1=%d\n", __func__,
		     cd->window.x0, cd->window.y0, cd->window.x1,
		     cd->window.y1);
...
static int thp_mt_wrapper_ioctl_get_projectid(struct thp_core_data *cd,
					      unsigned long arg)
{
...
static int thp_mt_wrapper_ioctl_get_res_scale(struct thp_core_data *cd,
					      unsigned long arg)
{
...
static int thp_mt_wrapper_ioctl_get_res_scale_pen(struct thp_core_data *cd,
						  unsigned long arg)
{
	...
	if (copy_to_user(res_scale_pen, &cd->res_scale_pen,
			 sizeof(cd->res_scale_pen))) {
...
static int thp_mt_wrapper_ioctl_get_screen_status(struct thp_core_data *cd,
						  unsigned long arg)
{
	...
	if (copy_to_user(fold_status, &cd->fold_status,
			 sizeof(cd->fold_status))) {
...
static int thp_mt_wrapper_ioctl_enable_pen(struct thp_core_data *cd,
					   unsigned long arg)
{
	...
	if (cd->suspended && thp_get_status(cd, THP_STATUS_STYLUS3) &&
		...
		if (cd->thp_dev && cd->thp_dev->ops &&
		    cd->thp_dev->ops->suspend_connect_pen) {
			cd->thp_dev->ops->suspend_connect_pen(cd->thp_dev);
			thp_log_info(cd, "%s:suspend connect pen\n", __func__);
		}
	}
...
static int thp_mt_wrapper_ioctl_disable_pen(struct thp_core_data *cd,
					    unsigned long arg)
{
	u32 __user *status = (u32 __user *)(uintptr_t)arg;
	...
	if (cd->suspended && (!thp_get_status(cd, THP_STATUS_STYLUS3))) {
		if (cd->thp_dev && cd->thp_dev->ops &&
		    cd->thp_dev->ops->suspend_disconnect_pen) {
			cd->thp_dev->ops->suspend_disconnect_pen(cd->thp_dev);
			thp_log_info(cd, "%s:suspend disconnect pen\n",
				     __func__);
		}
	}
...
static int thp_mt_wrapper_ioctl_set_roi_data(struct thp_core_data *cd,
					     unsigned long arg)
{
...
static long thp_mt_wrapper_ioctl_set_events(struct thp_core_data *cd,
					    unsigned long arg)
{
	void __user *argp = (void __user *)(uintptr_t)arg;
	int val;
...
static int thp_mt_ioctl_report_keyevent(struct thp_core_data *cd,
					unsigned long arg)
{
	...
	if (keyevent == INPUT_MT_WRAPPER_KEYEVENT_ESD) {
		...
		thp_log_info(cd,
			     "[Proximity_feature] %s: report [near] event!\n",
			     __func__);
     ...
     (defined CONFIG_HONOR_SENSORS_2_0))
		...
		thp_log_info(cd,
			     "[Proximity_feature] %s: report [far] event!\n",
			     __func__);
...
static long thp_mt_wrapper_ioctl_get_platform_type(struct thp_core_data *cd,
						   unsigned long arg)
{
...
static int thp_mt_wrapper_ioctl_set_large_object_event(struct thp_core_data *cd,
						       unsigned long arg)
{
	void __user *argp = (void __user *)(uintptr_t)arg;
	int data;
	...
	if (data) {
		input_report_key(input_dev, KEY_SET_LARGE_OBJECT_EVENT,
				 THP_KEY_DOWN);
		input_sync(input_dev);
		thp_log_info(cd, "%s: key down\n", __func__);
	} else {
		input_report_key(input_dev, KEY_SET_LARGE_OBJECT_EVENT,
				 THP_KEY_UP);
		input_sync(input_dev);
		thp_log_info(cd, "%s: key up\n", __func__);
	}

	return 0;
}

static long thp_mt_wrapper_ioctl_get_touchpad_info(struct thp_core_data *cd,
						   unsigned long arg)
{
	struct thp_touchpad_info __user *touchpad_info =
		(struct thp_touchpad_info *)(uintptr_t)arg;
	...
	thp_log_info(cd, "%s: touchpad_enable=%d,x0=%d,y0=%d,x1=%d,y1=%d\n",
		     __func__, cd->touchpad.touchpad_enable, cd->touchpad.x0,
		     cd->touchpad.y0, cd->touchpad.x1, cd->touchpad.y1);

	if (copy_to_user(touchpad_info, &cd->touchpad,
			 sizeof(struct thp_touchpad_info))) {
		thp_log_err(cd, "%s:copy touchpad_info failed\n", __func__);
		return -EFAULT;
	}
...
static long
thp_mt_wrapper_ioctl_set_touchpad_coordinate(struct thp_core_data *cd,
					     unsigned long arg)
{
	void __user *argp = (void __user *)(uintptr_t)arg;
	struct thp_mt_wrapper_ioctl_touch_data touchpad_data;
	struct input_dev *touchpad_dev = cd->thp_mt_wrapper->touchpad_dev;
	...
	if (!touchpad_dev) {
		thp_log_err(cd, "%s:touchpad_dev is null\n", __func__);
		return -EINVAL;
	}
	if (copy_from_user(&touchpad_data, argp, sizeof(touchpad_data))) {
		thp_log_err(cd, "Failed to copy_from_user()\n");
		return -EFAULT;
	}

	thp_coordinate_report(cd, touchpad_dev, touchpad_data);

	return 0;
}

static int thp_report_system_event(struct thp_core_data *cd,
				   struct thp_key_info *key_info)
{
	...
	input_report_key(input_dev, key_info->key, key_info->action);
	input_sync(input_dev);
	return 0;
}

static int thp_mt_ioctl_report_system_keyevent(struct thp_core_data *cd,
					       unsigned long arg)
{
	void __user *argp = (void __user *)(uintptr_t)arg;
	struct thp_key_info key_info;
...
#if (IS_ENABLED(CONFIG_HONOR_THP_QCOM) || IS_ENABLED(CONFIG_HONOR_THP_MTK))
static int thp_notify_fp_event(struct thp_core_data *cd,
			       struct thp_shb_info info)
{
	int ret = 0;
	...
	if ((info.cmd_type != THP_FINGER_PRINT_EVENT) &&
	    (info.cmd_type != THP_AUXILIARY_DATA)) {
		thp_log_info(
			cd,
			"%s: only handle fp event or auxiliary data, return\n",
			__func__);
		return NO_ERR;
	}
	thp_log_info(cd, "%s: called\n", __func__);
	...
	if (info.cmd_type == THP_FINGER_PRINT_EVENT)
		ret = fp_ops->fp_irq_notify(
			(struct tp_to_udfp_data *)info.cmd_addr);
	else if ((info.cmd_type == THP_AUXILIARY_DATA) &&
		 cd->support_25D_anti_fake)
		ret = fp_ops->fp_anti_fake_data_notify(
			(struct fp_data_head_t *)info.cmd_addr);
	else
...
#if (IS_ENABLED(CONFIG_HONOR_THP_QCOM) || IS_ENABLED(CONFIG_HONOR_THP_MTK))
void send_event_to_fingerprint_ud(struct thp_core_data *cd,
				  struct thp_udfp_data udfp_data)
{
	struct ud_fp_ops *fp_ops = NULL;
...
static int thp_charger_detect_notifier_callback(struct thp_core_data *cd,
...
static int
thp_charger_detect_notifier_callback_main(struct notifier_block *self,
					  unsigned long event, void *data)
{
...
#if (IS_ENABLED(CONFIG_HONOR_THP_QCOM) || IS_ENABLED(CONFIG_HONOR_THP_MTK))
#if IS_ENABLED(CONFIG_HONOR_COMMON_POWER)
static int thp_ctrl_event_call(struct thp_core_data *cd,
...
static void thp_nt_charger_notifier_register(struct thp_core_data *cd)
{
	...
	if (cd->multi_panel_index == SUB_TOUCH_PANEL)
		cd->charger_detect_notify.notifier_call =
			thp_ctrl_event_call_sub;
	else
		cd->charger_detect_notify.notifier_call =
			thp_ctrl_event_call_main;
...
#endif
#endif
int thp_charger_notifier_register(struct thp_core_data *cd)
{
	int rc = 0;
...
int thp_charger_notifier_unregister(struct thp_core_data *cd)
{
	int rc = 0;
...
#endif
#if ((defined CONFIG_HONOR_SHB_THP) || \
     (IS_ENABLED(CONFIG_HONOR_THP_QCOM) || IS_ENABLED(CONFIG_HONOR_THP_MTK)))
static int thp_mt_ioctl_cmd_shb_event(struct thp_core_data *cd,
				      unsigned long arg)
{
	void __user *argp = (void __user *)(uintptr_t)arg;
	int ret;
...
static long thp_ioctl_get_volume_side(struct thp_core_data *cd,
				      unsigned long arg)
{
...
static long thp_ioctl_get_power_switch(struct thp_core_data *cd,
				       unsigned long arg)
{
	void __user *status = (void __user *)(uintptr_t)arg;
...
static void thp_report_pen_event(struct thp_core_data *cd,
				 struct input_dev *input, struct thp_tool tool,
				 int pressure, int tool_type, int tool_value)
{
	...
	thp_log_debug(cd,
		      "%s:tool.tip_status:%d, tool_type:%d, tool_value:%d\n",
	...
	if (tool.expand_flag & THP_EXPAND_ENABLED_FLAG) {
		tool.expand_flag &= (~THP_EXPAND_ENABLED_FLAG);
		input->absinfo[ABS_TP_EXPAND_FLAG].value = ~tool.expand_flag;
		input_report_abs(input, ABS_TP_EXPAND_FLAG, tool.expand_flag);
	}
...
static int thp_mt_wrapper_ioctl_report_pen(struct thp_core_data *cd,
					   unsigned long arg)
{
...
static long thp_ioctl_get_stylus3_connect_status(struct thp_core_data *cd,
						 unsigned long arg)
{
...
static int set_stylus3_change_protocol(struct thp_core_data *cd,
				       unsigned long arg)
{
...
static int thp_ioctl_set_stylus3_connect_status(struct thp_core_data *cd,
						unsigned long arg)
{
	void __user *argp = NULL;
...
static long thp_ioctl_get_grip_data(unsigned long arg)
{
	...
	if (ret) {
		...
		thp_log_info(
			cd, "%s, screen_rotate=%d, grip_type=%d, grip_num=%d\n",
...
static long thp_ioctl_get_callback_events(struct thp_core_data *cd,
					  unsigned long arg)
{
	void __user *argp = (void __user *)(uintptr_t)arg;
	int ret;
	...
	if (ret) {
		...
		thp_log_info(cd,
			     "%s, eventClass=%d, eventCode=%d, extraInfo=%s\n",
...
static int thp_ioctl_set_callback_events(struct thp_core_data *cd,
					 unsigned long arg)
{
	void __user *argp = (void __user *)(uintptr_t)arg;
	...
	if (copy_from_user(&cd->stylus3_callback_event, argp,
			   sizeof(cd->stylus3_callback_event))) {
		thp_log_err(cd, "%s Failed to copy_from_user\n", __func__);
		return -EFAULT;
	}
...
static int daemon_init_protect(struct thp_core_data *cd, unsigned long arg)
{
	...
	if (daemon_flag) {
		...
		if (atomic_read(&(cd->resend_suspend_after_fw_update)) == 1) {
			thp_log_info(
				cd,
				"%s: fw update complete, need resend suspend cmd\n",
				__func__);
				 ...
				 unsigned long arg)
{
	...
	case INPUT_MT_WRAPPER_IOCTL_GET_PLATFORM_TYPE:
		...
		break;
	case INPUT_MT_WRAPPER_IOCTL_SET_LARGE_OBJECT_EVENT:
		ret = thp_mt_wrapper_ioctl_set_large_object_event(cd, arg);
		break;
	case INPUT_MT_WRAPPER_IOCTL_GET_TOUCHPAD_INFO:
		ret = thp_mt_wrapper_ioctl_get_touchpad_info(cd, arg);
		break;
	case INPUT_MT_WRAPPER_IOCTL_CMD_SET_TOUCHPAD_COORDINATES:
		ret = thp_mt_wrapper_ioctl_set_touchpad_coordinate(cd, arg);
		break;
	...
	case INPUT_MT_WRAPPER_IOCTL_GET_SCREEN_FOLDED:
		...
		break;
	case INPUT_MT_WRAPPER_IOCTL_ENABLE_PEN:
		ret = thp_mt_wrapper_ioctl_enable_pen(cd, arg);
		break;
	case INPUT_MT_WRAPPER_IOCTL_DISABLE_PEN:
		ret = thp_mt_wrapper_ioctl_disable_pen(cd, arg);
		break;
	default:
...
static int thp_projectid_to_vender_id(struct thp_core_data *cd,
				      const char *project_id,
				      unsigned int project_id_len,
				      char *temp_buf, unsigned int len)
{
	...
	if ((strlen(project_id) > project_id_len) ||
		...
		thp_log_err(cd,
			    "%s:project_id or temp_buf has a wrong length\n",
			    __func__);
		return -EINVAL;
	}
	strncpy(temp_buf,
		...
		THP_PROJECTID_VENDOR_ID_LEN);

	return 0;
}

static void thp_set_inputdev_params(struct thp_core_data *cd,
				    struct input_dev *input_dev,
				    struct thp_input_dev_config *config)
{
	input_set_abs_params(input_dev, ABS_X, 0, config->abs_max_x - 1, 0, 0);
	input_set_abs_params(input_dev, ABS_Y, 0, config->abs_max_y - 1, 0, 0);
	if (cd->support_dynamic_resolution) {
		input_set_abs_params(input_dev, ABS_MT_POSITION_X, 0,
				     config->abs_max_x * cd->res_scale - 1, 0,
				     0);
		input_set_abs_params(input_dev, ABS_MT_POSITION_Y, 0,
				     config->abs_max_y * cd->res_scale - 1, 0,
				     0);
	} else {
		input_set_abs_params(input_dev, ABS_MT_POSITION_X, 0,
				     config->abs_max_x - 1, 0, 0);
		input_set_abs_params(input_dev, ABS_MT_POSITION_Y, 0,
				     config->abs_max_y - 1, 0, 0);
	}
	input_set_abs_params(input_dev, ABS_MT_PRESSURE, 0, config->abs_max_z,
			     0, 0);
	input_set_abs_params(input_dev, ABS_MT_TRACKING_ID, 0,
			     config->tracking_id_max - 1, 0, 0);
	input_set_abs_params(input_dev, ABS_MT_TOUCH_MAJOR, 0,
			     config->major_max, 0, 0);
	input_set_abs_params(input_dev, ABS_MT_TOUCH_MINOR, 0,
			     config->minor_max, 0, 0);
	input_set_abs_params(input_dev, ABS_MT_ORIENTATION,
			     config->orientation_min, config->orientation_max,
			     0, 0);
	input_set_abs_params(input_dev, ABS_MT_BLOB_ID, 0,
			     INPUT_MT_WRAPPER_MAX_FINGERS, 0, 0);
	input_set_abs_params(input_dev, ABS_TP_EXPAND_FLAG, 0,
			     THP_EXPAND_FLAG_MAX, 0, 0);
}
static int thp_parse_input_config(struct thp_core_data *cd,
				  struct thp_input_dev_config *config)
{
	int rc;
	unsigned int i;
	...
	if (cd->support_diff_resolution) {
		...
		for (i = 0; i < ARRAY_SIZE(thp_input_compatible_table); i++) {
			if (!strncmp(thp_input_compatible_table[i].vendor_id,
				     (const char *)temp_buf,
				     strlen(thp_input_compatible_table[i]
						    .vendor_id))) {
				thp_dev_node = of_find_compatible_node(
					NULL, NULL,
					thp_input_compatible_table[i]
						.compatible_name);
				break;
			}
		}
		...
		if (i == ARRAY_SIZE(thp_input_compatible_table)) {
			thp_log_err(cd, "%s:vendor id:%s not in id_table\n",
				    __func__, temp_buf);
			thp_dev_node = of_find_compatible_node(
				NULL, NULL, THP_INPUT_DEV_COMPATIBLE);
		}
	} else {
		thp_dev_node = of_find_compatible_node(
			cd->thp_node, NULL, THP_INPUT_DEV_COMPATIBLE);
	}
	if (!thp_dev_node) {
...
static int thp_parse_pen_input_config(struct thp_core_data *cd,
				      struct thp_input_pen_dev_config *config)
{
	int rc = -EINVAL;
	struct device_node *thp_dev_node = NULL;
	...
	rc = of_property_read_u32(thp_dev_node, "max_x", &config->max_x);
	if (rc) {
		thp_log_err(cd, "%s:max_x not config\n", __func__);
		goto err;
	}

	rc = of_property_read_u32(thp_dev_node, "max_y", &config->max_y);
	if (rc) {
...
static int thp_set_pen_input_config(struct thp_core_data *cd,
				    struct input_dev *pen_dev)
{
	...
	if (cd->support_dynamic_resolution) {
		input_set_abs_params(
			pen_dev, ABS_X, 0,
			cd->thp_mt_wrapper->input_pen_dev_config.max_x *
				cd->res_scale_pen,
			0, 0);
		input_set_abs_params(
			pen_dev, ABS_Y, 0,
			cd->thp_mt_wrapper->input_pen_dev_config.max_y *
				cd->res_scale_pen,
			0, 0);
	} else {
		input_set_abs_params(
	...
	input_set_abs_params(pen_dev, ABS_PRESSURE, 0,
			     cd->thp_mt_wrapper->input_pen_dev_config.pressure,
			     0, 0);
	input_set_abs_params(
		pen_dev, ABS_TILT_X,
	...
	input_set_abs_params(pen_dev, ABS_TP_EXPAND_FLAG, 0,
			     THP_EXPAND_FLAG_MAX, 0, 0);
...
int thp_lcd_status_notifier(struct thp_core_data *cd, int type)
{
	int ret = 1;
	struct lcd_kit_ops *kit_ops = lcd_kit_get_ops();
	if ((kit_ops) && (kit_ops->get_thp_notify_lcd_by_type)) {
		ret = kit_ops->get_thp_notify_lcd_by_type(type);
		if (ret < 0) {
			thp_log_info(
				cd, "%s: raod_power_status_notifier failed!!\n",
				__func__);
			return ret;
		}
	}
...
int touchpad_input_device_register(struct thp_core_data *cd)
{
	int rc;
	char *node = NULL;
	struct thp_input_dev_config *touchpad_config = NULL;
	struct input_dev *touchpad_dev = input_allocate_device();
	...
	if (cd->multi_panel_index == 0) {
		touchpad_dev->id.product = INTERNAL_TOUCHPAD_PRODUCT_ID;
	} else {
		thp_log_err(cd, "%s:panel_index is not 0\n", __func__);
		rc = -EINVAL;
		goto err_free_dev;
	}

	touchpad_dev->name = TS_TOUCHPAD_DEV_NAME;
	touchpad_dev->id.vendor = DEFAULT_VENDOR_ID;
	touchpad_dev->id.version = DEFAULT_VERSION_ID;
	...
	rc = snprintf(node, MULTI_PANEL_NODE_BUF_LEN, "%s%d",
		      TS_TOUCHPAD_DEV_NAME, cd->multi_panel_index);
	if (rc < 0) {
		thp_log_err(cd, "%s: snprintf err\n", __func__);
		goto err_free_dev;
	}

	touchpad_dev->name = (const char *)node;
	thp_log_info(cd, "%s touchpad_dev->name is :%s\n", __func__,
		     touchpad_dev->name);
	cd->thp_mt_wrapper->touchpad_dev = touchpad_dev;
	cd->thp_mt_wrapper->touchpad_dev_config =
		cd->thp_mt_wrapper->input_dev_config;
	touchpad_config = &cd->thp_mt_wrapper->touchpad_dev_config;

	__set_bit(EV_SYN, touchpad_dev->evbit);
	__set_bit(EV_KEY, touchpad_dev->evbit);
	__set_bit(EV_ABS, touchpad_dev->evbit);
	__set_bit(BTN_TOUCH, touchpad_dev->keybit);
	__set_bit(INPUT_PROP_POINTER, touchpad_dev->propbit);

	thp_set_inputdev_params(cd, touchpad_dev, touchpad_config);
	rc = input_register_device(touchpad_dev);
	if (rc) {
		thp_log_err(cd, "%s:failed to register touchpad device: %d\n",
			    __func__, rc);
		goto err_free_dev;
	}
	return rc;
...
static int thp_input_pen_device_register(struct thp_core_data *cd)
{
	...
	if (cd->multi_panel_index == 0)
		pen_dev->id.product = INTERNAL_PEN_PRODUCT_ID;
	else if (cd->multi_panel_index == 1)
		pen_dev->id.product = EXTERNAL_PEN_PRODUCT_ID;
	pen_dev->name = TS_PEN_DEV_NAME;
	pen_dev->id.vendor = DEFAULT_VENDOR_ID;
	pen_dev->id.version = DEFAULT_VERSION_ID;
...
int thp_mt_wrapper_input_dev_register(struct thp_core_data *cd)
{
	...
	if (cd->multi_panel_index == 0)
		input_dev->id.product = INTERNAL_PANEL_PRODUCT_ID;
	else if (cd->multi_panel_index == 1)
		input_dev->id.product = EXTERNAL_PANEL_PRODUCT_ID;
	else
		input_dev->id.product = SINGLE_PANEL_PRODUCT_ID;
	input_dev->name = THP_INPUT_DEVICE_NAME;
	input_dev->id.vendor = DEFAULT_VENDOR_ID;
	input_dev->id.version = DEFAULT_VERSION_ID;
	...
	__set_bit(KEY_SET_LARGE_OBJECT_EVENT, input_dev->keybit);
	__set_bit(KEY_FULLAOD_SLIDE_EVENT, input_dev->keybit);

	thp_set_inputdev_params(cd, input_dev, &mt_wrapper->input_dev_config);
	if (cd->support_b_protocol) {
...
int thp_mt_wrapper_init(struct thp_core_data *cd)
{
	...
	if (cd->touchpad_supported) {
		rc = touchpad_input_device_register(cd);
		if (rc)
			thp_log_err(cd, "%s:touchpad register failed\n",
				    __func__);
	}
...
#endif //(defined CONFIG_HONOR_THP_WRAPPER)
