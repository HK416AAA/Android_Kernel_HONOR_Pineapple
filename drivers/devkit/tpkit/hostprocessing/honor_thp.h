...
enum lcd_kit_ts_pm_type {
	...
	TS_IC_SHUT_DOWN,
	TS_2ND_POWER_OFF,
	TS_EVENT_FINGER_DOWN,
	TS_EVENT_FINGER_UP,
	TS_EVENT_HOVER_DOWN,
	TS_EVENT_HOVER_UP,
	TS_EVENT_DOUBEL_CLICK,
	TS_DOUBLE_CLICK_SWITCH_ON,
	TS_DOUBLE_CLICK_SWITCH_OFF,
	TS_AOD_ON_RESUME,
	TS_AOD_ON_SUSPEN,
	TS_AOD_OFF_RESUME_SUSPEN,
	TS_AFE_SCREEN_ON,
};
#endif
...
#define MAX_EVENTS 5
#define RESERVE_MAX_LENGTH 24
#define FINGERPRINT_SIGNAL_LENGTH 9

#define THP_SPI_CMD_LOCK 1
#define THP_SPI_CMD_UNLOCK 0
#define SPI_CMD_LOCK_TIMEOUT 50
...
enum touch_panel_ic_type { IC_TYPE_GOODIX = 1, IC_TYPE_SYNA = 2 };
...
#define THP_IOCTL_CMD_SET_SPI_CMD_LOCK _IOWR(THP_IO_TYPE, 0x14, u32)
...
#define KEY_SET_LARGE_OBJECT_EVENT 612
#define KEY_FULLAOD_SLIDE_EVENT 613
#define MULTI_PANEL_NODE_BUF_LEN 100
#define TP_TOUCHSCREEN_NODE_NAME "touchscreen"

#define GET_FRAME_RETRY_COUNT_MAX 3

#define IRQ_SCHEDULER_OP_BOOST 1
#define IRQ_SCHEDULER_OP_RESTORE 0
#define IRQ_SCHEDULER_BOOST_THRESHOLD 12
#define IRQ_SCHEDULER_BOOST_HOLD_TIME 10000
...
#define NOTIFY_TP_STANDBY_IN 3
#define NOTIFY_TP_STANDBY_OUT 4
#define INVALID_VALUE 0
#define AOD_VALID_EVENT 1
#define VALID_SLIDE_EVNET 1
...
struct input_mt_wrapper_touch_data {
	...
	int minor;
	int orientation;
	unsigned int tool_type : 16;
	unsigned int expand_flag : 16;
};

struct click_unit_struct {
	unsigned short type; /* click 1/double click 2/fingerprint 3/slide 4 */
	unsigned short click_reason; /* reason for success/failure */
	/*
         * click 1: success 0, timeout 1, overrange 2
         * double-click 2: 0 on success. On failure, two tap events are directly stored in the buffer.
         * fingerprint 3: success 0, area failure 1 (success refers to bright spots in the
         * fingerprint area, not successful fingerprint unlocking)
         * slide 4: success 0, timeout 1, overrange 2, overspeed 3 (click to change to slide)
         * unknown type event-type5, click_reason defaults to 0
         */
	unsigned short x;
	/*
         * fill in the x-coordinate for a light touch / fill in the x-coordinate of the second tap
         * event when the double-click is successful / fill in the X-direction movement distance
         * for a sliding event (whether successful or not): |X_up-X_down|
         */
	unsigned short y;
	/*
         * Fill in the y coordinate of a tap / fill in the y coordinate of the second tap event
         * when the double click is successful / fill in the Y coordinate of the sliding event
         * (whether successful or not): |Y_up-Y_down|
         */
	unsigned short count;
	/* process count // count +1 in each loop, used to indirectly display the time value */
};

struct debug_info_struct {
	unsigned short finger_num;
	/*
         * current finger num Number of fingers when reporting an event
         * (1 when Synaptics fully covers the fingerprint area, 0 for light touch/double click)
         */
	short abs_max; /* max abs self capacitance diff */
	short abs_min; /* min abs */
	short trans_max; /* max trans self capacitance diff */
	short trans_min; /* max min */
	unsigned short max_raw;
	/* max raw   mutual capacitance raw       synaptics fill in 0 */
	unsigned short min_raw;
	/* min raw                 synaptics fill in 0 */
	unsigned short finger_threshold;
	/* finger threshold  diff-finger dynamic threshold */
	unsigned short hope_freq_count;
	/* hope_freq count frequency hopping times */

	unsigned short baseline_update_count;
	/* baseline slow relax 0-2000，clear when beyond 2000 */
	unsigned short baseline_reset_count;
	/* baseline fast relax 0-2000，clear when beyond 2000 */
	unsigned short baseline_change_reason; /* baseline change reason */
	unsigned short current_freq; /* current work frequency */
	short noise; /* current noise */

	struct click_unit_struct click[MAX_EVENTS];

	unsigned short fw_status; /* bit0 Active/Idle,bit1 Charger bit */
	short fingerprint_signal[FINGERPRINT_SIGNAL_LENGTH];
	/*
         * fingerprint signal    The 3×3 fingerprint area corresponds to diff and
         * corresponds to the data when the event was reported.
         */
	unsigned short fingerprint_area;
	/* finger print cover status  sensor num     synaptics has four situations: 0-no coverage,
         * 1-coverage error, 2-partial coverage, 3-full coverage, determined
         *based on fingerprintdown_sensornumber
         */
	unsigned short fingerprintdown_sensornumber;
	/* finger print cover sensor num  number of sensors covering the fingerprint area */
	unsigned short int_count;
	/* report index  number of interruptions,
         * synaptics (one detection, one transmission) count+2
         */

	unsigned short reserve[RESERVE_MAX_LENGTH];
};

struct thp_slide_position_report {
	bool support_report_flag;
...
struct thp_udfp_data {
	...
	unsigned int slide_event;
	struct thp_slide_position_report slide_position;
	//standby mult fingers
	struct input_mt_wrapper_touch_data
		coor_data[INPUT_MT_WRAPPER_MAX_FINGERS];
};
...
struct thp_device_ops {
	...
	int (*get_project_id)(struct thp_device *tdev, char *buf,
			      unsigned int len);
	...
	int (*chip_wakeup_gesture_enable_switch)(struct thp_device *tdev,
						 u8 switch_value);
	int (*tui_enable_switch)(struct thp_device *tdev, u8 switch_value);
	...
	int (*get_active_doze_time)(struct thp_device *tdev,
	...
	int (*standby_mode_exit_lowpower)(struct thp_device *tdev);
	int (*tp_aod_event_ctrl)(struct thp_device *tdev, u8 status,
				 struct thp_aod_window window);
	void (*tp_fullaod_gesture_support)(struct thp_device *tdev, u8 status);
#endif
	int (*spi_transfer_one_byte_bootloader)(struct thp_device *tdev,
...
#endif
	int (*get_tui_event_info)(struct thp_device *tdev,
				  struct thp_udfp_data *udfp_data);
	int (*suspend_connect_pen)(struct thp_device *tdev);
	int (*suspend_disconnect_pen)(struct thp_device *tdev);
	int (*prepare)(struct thp_device *tdev);
	void (*thp_cmd_proc)(struct thp_device *tdev,
...
struct thp_gpios {
	...
	int rst_gpio;
	int cs_gpio;
	/* For FCP, controls if ir is connected to the MOSI pin of SPI */
	int shared_control_gpio;
};
...
struct thp_cmd_param {
	union {
		...
		unsigned int scheduler_boost_op;
...
enum thp_cmd {
	...
	TS_GET_ACTIVE_DOZE_RATIO,
	THP_TOUCH_DRIVER_CMD,
	TS_STANDBY_IN,
	TS_STANDBY_OUT,
	AOD_NOTIFY_SUBPANEL_STANDBY_IN,
	AOD_NOTIFY_SUBPANEL_STANDBY_OUT,
	AOD_NOTIFY_FULLAOD_DOZE_STATUS,
	IRQ_SCHEDULER_BOOST,
	NOTIFY_TP_LOWPOWER_STATUS,
	TS_INVAILD_CMD = 255,
};
...
struct thp_irq_scheduler_boost {
...
struct thp_core_data {
	...
	bool suspended;
	bool standby_mode;
	u32 in_standby_state;
	u32 standby_enable;
	u32 fullaod_support_gesture;
	bool fullaod_tp_need_work;
	struct mutex suspend_flag_mutex;
	bool early_suspended;
	u8 work_status;
	u32 spi_autosleep_short_delay;
	u32 touchpad_supported;
	u32 support_get_active_doze_time;
#if IS_ENABLED(CONFIG_DSM)
	...
	struct thp_touchpad_info touchpad;
	...
	bool unfold_restart_ignore_after_resume;
	bool fold_force_power_off;
	...
	u32 phone_call_aft_event_status;
	...
	struct completion thp_probe_completion;
	u16 frame_data_addr;
	u32 support_shb_thp;
	...
	u32 support_pad_frame_len_max_size;
	u32 lowpower_support;
	u32 is_fold_device;
	u32 irq_boost_touch;
	u32 fold_status;
	u32 sub_solution;
	u32 get_frame_timeout_retry_count;
	...
	int ic_type;
	u32 support_get_project_id_late;
#if IS_ENABLED(CONFIG_HONOR_THP_QCOM)
	...
	struct thp_vm_core *vm_core;
	struct thp_spi_cmd_lock cmd_lock;
	struct thp_irq_scheduler_boost scheduler_boost;
};
	...
	do {                                                            \
		if (cd) {                                               \
			if (cd->multi_panel_index == SUB_TOUCH_PANEL) { \
				printk(KERN_ERR "[E/THP1] " msg,        \
				       ##__VA_ARGS__);                  \
			} else {                                        \
				printk(KERN_ERR "[E/THP0] " msg,        \
				       ##__VA_ARGS__);                  \
			}                                               \
		} else {                                                \
...
#define thp_log_info(cd, msg, ...)                                      \
	do {                                                            \
		if (cd) {                                               \
			if (cd->multi_panel_index == SUB_TOUCH_PANEL) { \
				printk(KERN_ERR "[I/THP1] " msg,        \
				       ##__VA_ARGS__);                  \
			} else {                                        \
				printk(KERN_ERR "[I/THP0] " msg,        \
				       ##__VA_ARGS__);                  \
			}                                               \
		} else {                                                \
		...
		if (g_thp_log_cfg) {                                     \
			if (cd) {                                        \
				if (cd->multi_panel_index ==             \
				    SUB_TOUCH_PANEL) {                   \
					printk(KERN_ERR "[D/THP1] " msg, \
					       ##__VA_ARGS__);           \
				} else {                                 \
					printk(KERN_ERR "[D/THP0] " msg, \
					       ##__VA_ARGS__);           \
				}                                        \
			} else {                                         \
				printk(KERN_ERR "[D/THP] " msg,          \
				       ##__VA_ARGS__);                   \
			}                                                \
		}                                                        \
	} while (0)
...
void thp_pen_send_detect_cmd(struct thp_core_data *cd, struct thp_device *dev,
			     int timeout);
int thp_pen_parse_timing_config(struct thp_core_data *cd,
				struct device_node *timing_cfg_node,
				struct thp_timing_config *timing);
...
static inline bool in_irq_thread(struct thp_core_data *cd)
{
	return current->pid == cd->irq_thread_pid;
}
